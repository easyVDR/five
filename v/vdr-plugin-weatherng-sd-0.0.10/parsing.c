...
#define ATTRIB_MAN_STRING(_attr,_target) \
        ATTRIB_OPT_STRING(_attr,_target) \
                ...
                esyslog("ERROR : weatherng-sd: Mandatory attribute %s missing in tag %s", \
                                _attr, name.c_str()); \
                return false; \
        }
...
bool xCharData(const std::string &text) {
	if(context.size()>0 ) {
		...
		if (context[context.size()-1]=="hi" && IsDay==true){
                        dsyslog("DEBUG : weatherng-sd: hi Wert: %s\n",text.c_str());
	...
	if(context.size()>0 ) {
   	   	if (context[context.size()-1]=="dnam"){
                        dsyslog("DEBUG : weatherng-sd: dnam: %s\n",text.c_str());
			town=text.c_str();
                }

   	   	if (context[context.size()-1]=="ut"){
                        dsyslog("DEBUG : weatherng-sd: unit 1: %s\n",text.c_str());
			unit1=text.c_str();
                }
   	   	if (context[context.size()-1]=="us"){
                        dsyslog("DEBUG : weatherng-sd: unit 2: %s\n",text.c_str());
			unit2=text.c_str();
                }
...
void cxmlParse::xmlParse(int daycount, std::string DataDir, std::string data_file, int inDPart) {
		...
		if (xml.parse() != 0) {
		    esyslog("ERROR : weatherng-sd: Can not parse %s\n",data_file.c_str());
		}else{
