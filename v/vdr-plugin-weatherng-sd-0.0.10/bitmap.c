	...
	if (mCache.Contains(spec)) {
		...
		if ((pos = fname.find('*')) != -1) {
			...
			if (glob(fname.c_str(), 0, NULL, &gbuf) == 0){
				...
				if (!Quiet)
					esyslog("weatherng-sd-image: ERROR: No match for wildcard filename %s", 
							Filename.c_str());
				fname = "";
			} 
			globfree(&gbuf);
		...
		if (result) {
		  ...
		  esyslog("weatherng-sd-image: ERROR: filename %s too short to identify format", fname.c_str());
		  DELETENULL(res);
		}	
		mCache[spec] = res;
	}
...
bool cWeatherBitmap::LoadMagick(const char *Filename, int height, int width, int colors, bool Quiet) {
  ...
  try {
    ...
    if(images.size() == 0)	{
      esyslog("weatherng-sd-image: ERROR: images.size == NULL . could not load %s", Filename);
      return false;
    }

    printf("weatherng-sd-image: load image %s\n", Filename);
    ...
    if (!Quiet)	esyslog("weatherng-sd-image: ERROR: Couldn't load %s: %s", Filename, e.what());
    delete bmp;
    return false;
  }
  catch (...) {
    if (!Quiet)	esyslog("weatherng-sd-image: ERROR: Couldn't load %s: Unknown exception caught", Filename);
    delete bmp;
    return false;
  }
...
bool cWeatherBitmap::LoadImlib(const char *Filename, int height, int width, int colors, bool Quiet) {
  ...
  printf("weatherng-sd-image: Loading '%s' with parameters: h:'%i' w:'%i' colors:'%i'\n", Filename, height, width, colors);
  ...
  if (!buffer) {
    esyslog("weatherng-sd-image: ERROR: buffer empty. ...aborting!");
    delete(quantizer);
    return false;
  }
