...
my $path = "/etc/vdr/plugins/graphTFT/themes/avp/scripts";
my $inpath0 = "/etc/vdr/plugins/graphTFT/themes/avp/png0";
my $inpath1 = "/etc/vdr/plugins/graphTFT/themes/avp/png1";
my $linkpath = "/etc/vdr/plugins/graphTFT/themes/avp/columnimages";
...
while (<FILE>) {
    ...
    if ($png0{$searchname}) {
	$cnt++;
	$status = symlink("./../png0/$png0{$searchname}","$linkpath/$channame.png");
	if ($status == 1)  { print LOG "$channame => ./../png0/$png0{$searchname}"; }
	else { print LOG "$channame => failed"; } 
	if ($shortname and $shortname ne '') {
	    $status = symlink("./../png0/$png0{$searchname}","$linkpath/$shortname.png");
