  ...
  switch (data[0]) { // data_unit_id
      case 0xC3: // VPS line 16
         ...
         if (cni_vps && GetCniNameVPS()) {
            if (CNI_DBG)
               dlog(4, "cni_vps = 0x%.3x -> %s", cni_vps, GetCniNameVPS());
                   ...
                   if (mode_description == 0x08) {
                       ...
                       if (data_word_A >= 0x30) {
                           ...
                           if (cni_X_26 && GetCniNameX26()) {
                              if (CNI_DBG)
                                 dlog(4, "cni_X_26 = 0x%.4x -> %s", cni_X_26, GetCniNameX26());
                              if (++hits > MAXHITS) stopped = true;
                              }
                           }
                       }
               ...
               switch (DesignationCode) {
                   ...
                   case 1:
                      ...
                      if (cni_8_30_1 && GetCniNameFormat1())
                         if (CNI_DBG)
                            dlog(4, "cni_8_30_1 = 0x%.4x -> %s", cni_8_30_1, GetCniNameFormat1());
                         if (++hits > MAXHITS) stopped = true;
                      }
                      break;
                   case 2:
                   case 3:
                      ...
                      if (cni_8_30_2 && GetCniNameFormat2())
                         if (CNI_DBG)
                            dlog(4, "cni_8_30_2 = 0x%.4x -> %s", cni_8_30_2, GetCniNameFormat2());
                         if (++hits > MAXHITS) stopped = true;
                      }
                      break;
                   default:;
...
void cSwReceiver::Action() {
  ...
  if (hits < MAXHITS) {
     ...
     if ((hits < MINHITS) || (TsCount < 3)) {
        ...
        dlog(4, "   fuzzy result, hits = %d, TsCount = %lu", hits, TsCount);   
     }
}
