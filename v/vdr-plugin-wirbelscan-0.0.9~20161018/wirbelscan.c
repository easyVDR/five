...
cString cPluginWirbelscan::SVDRPCommand(const char *Command, const char *Option, int &ReplyCode)
{
  ...
  else if cmd("S_PVR"   ) { return DoScan(wSetup.DVB_Type = SCAN_PVRINPUT)      ? "PVRx50 scan started"    : "Could not start PVRx50 scan.";   }
  else if cmd("S_PVR_FM") { return DoScan(wSetup.DVB_Type = SCAN_PVRINPUT_FM)   ? "PVRx50 FM scan started" : "Could not start PVRx50 FM scan.";}
