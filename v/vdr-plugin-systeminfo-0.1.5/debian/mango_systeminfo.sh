...
case "$1" in
	...
	6)	# fan speeds
		...
		CASE=$( sensors | grep -i 'fan4:' | tr -s ' ' | cut -d' ' -f 2)
		echo -ne "Fans:\tCPU: "$CPU" rpm\tCase: "$CASE" rpm"
		exit
        	;;

	7)	# temperature of CPU and mainboard
		CPU=$(sensors | grep -i 'temp1\|cpu temp\|cpu0 temp\|Core 1:' | head -n1 | sed 's/.*: *\(\S*\).*/\1/')
		MB=$(sensors | grep -i 'temp2\|m/b temp\|system temp\|Core 0:' | head -n1 | sed 's/.*: *\(\S*\).*/\1/')
		echo -ne "Temperatur:\tCPU: "$CPU"\tMB: "$MB
		exit
        	;;
