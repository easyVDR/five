...
static int _write_osdcmd(cxSocket& s, osd_command_t *cmd)
{
  size_t len = 0;
  uint8_t buf[sizeof(osd_command_t)];

#define PACK_FIELD(FIELD)                                               \
  do {                                                                  \
    memcpy(buf + len, &cmd->FIELD, sizeof(cmd->FIELD));                 \
    len += sizeof(cmd->FIELD);                                          \
  } while (0)
  PACK_FIELD(size);
  PACK_FIELD(cmd);
  PACK_FIELD(wnd);
  ...
  PACK_FIELD(colors);
  PACK_FIELD(dummy02);
  PACK_FIELD(dirty_area.x1);
  PACK_FIELD(dirty_area.y1);
  PACK_FIELD(dirty_area.x2);
  PACK_FIELD(dirty_area.y2);
  PACK_FIELD(flags);
  PACK_FIELD(scaling);
#undef PACK_FIELD
  ...
  if((ssize_t)len != s.write(buf, len, 100)) {
    return -1;
  }
  return 0;
}
...
void cXinelibServer::Handle_Control_RTSP(int cli, const char *arg)
{
  ...
  //
    ...
    //
      ...
      if(accept && strstr(accept->Value(), SDP_MIME_TYPE)) {
        union {
          struct sockaddr    sa;
          struct sockaddr_in in;
        } addr;
