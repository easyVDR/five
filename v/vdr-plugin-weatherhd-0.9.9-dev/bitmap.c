...
{
#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c : check for image '%s'\n", Filename);
#endif

  if(FileExists(Filename)) {
#ifdef DEBUG_COVER
    printf("weatherhd: bitmap.c : image found\n");
#endif
    return true;
    }
  else {
#ifdef DEBUG_COVER
    printf("weatherhd: bitmap.c : imagefile dont exists\n");
#endif
    esyslog("weatherhd: bitmap.c : imagefile dont exists");
    return false;
  }
...
#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c : load image '%s'\n",Filename);
  printf("weatherhd: bitmap.c : values: width=%d height=%d ratio=%d startx=%d isProgressbar=%d\n", width, height, ratio, startx, isProgressbar );
#endif
...
#ifdef DEBUG_COVER
    printf("weatherhd: bitmap.c: no image or wrong dimension\n");
#endif
    esyslog("weatherhd: bitmap.c: no image or wrong dimension");
    }
  else {
...
#ifdef DEBUG_COVER
      printf("weatherhd: bitmap.c: create a progressbar\n");
#endif
      }
    else {
      if(!ratio) {
...
#ifdef DEBUG_COVER
        printf("weatherhd: bitmap.c: create an image without aspect ratio:\nimlib_image_get_width=%i imlib_image_get_height=%i\n", imlib_image_get_width(), imlib_image_get_height());
#endif
        }
      else {
        imlib_context_set_image(image);
...
#ifdef DEBUG_COVER
        printf("weatherhd: bitmap.c: create an image with aspect ratio x1=%i y1=%i x2=%i y2=%i\n", x1 ,y1 ,x2, y2);
#endif
...
#ifdef DEBUG_COVER
    printf("weatherhd: bitmap.c: new values of image: width=%d  height=%d\n", newwidth, newheight);
#endif
...
#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c: start to render image\n");
#endif
...
#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c: image rendered !\n");
#endif

#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c: imlib_free_image()\n");
#endif
  if(image) imlib_free_image();


#ifdef DEBUG_COVER
  printf("weatherhd: bitmap.c: imlib_context_free()\n");
#endif
...
void cImageloadBitmap::LoadImage( const char *Filename, int width, int height, bool ratio, int startx, bool isProgressbar)
{
  ...
  if ( height <=0 || width <=0) {
...
#ifdef DEBUG_COVER
    printf("weatherhd: bitmap.c: original image dimension = %dx%d\n", image.columns(), image.rows());
#endif
      ...
      if(isProgressbar) {
        ...
        printf("weatherhd: bitmap.c: create an image without aspect ratio\n");
          ...
          image.sample(geometry);
#ifdef DEBUG_COVER
          printf("weatherhd: bitmap.c: create an image with aspect ratio\n");
#endif
...
#ifdef DEBUG_COVER
      printf("weatherhd: bitmap.c: new image dimensions = %dX%d\n", newwidth, newheight);
#endif
    ...
    {
      esyslog("weatherhd: bitmap.c: Could not load %s: %s", Filename, e.what());
    }
  }
