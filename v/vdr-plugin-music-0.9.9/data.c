...
bool CheckVDRVersion(int Version, int Major, int Minor, const char *text)
{
  ...
  if(sscanf(vv,"%d.%d.%d",&version,&major,&minor)==3) {
    if(version<Version ||
      ...
      if(text) {
        esyslog("music: ERROR: %s plugin needs at least VDR version %d.%d.%d",text,Version,Major,Minor);
        fprintf(stderr,"%s plugin needs at least VDR version %d.%d.%d\n",text,Version,Major,Minor);
        }
      return false;
      }
    }
  else esyslog("music: ERROR: cannot parse VDR version string '%s'",vv);
  return true;
}
...
bool cScanDir::ScanDir(cFileSource *src, const char *subdir, eScanType type, const char * const *spec, const char * const *excl, bool recursiv)
{
  ...
  if(d) {
    ...
    while((e=readdir64(d))) {
      ...
      struct stat64 st;
      if(stat64(f,&st)<0) {
        esyslog("music: ERROR: stat(1) %s: %s",f,strerror(errno));
        continue;
        }
      if(S_ISLNK(st.st_mode)) {
        ...
        if(!f) continue;
        if(stat64(f,&st)<0) {
          esyslog("music: ERROR: stat(2) %s: %s",f,strerror(errno));
          continue;
          }
        }
      ...
      if(S_ISREG(st.st_mode)) {
        ...
        if(spec) {
          ...
          for(const char * const *m=spec; *m; m++) {
            ...
            if(n!=FNM_NOMATCH) esyslog("music: ERROR: fnmatch(1) %s: %s",*m,strerror(errno));
            }
          if(!ok) continue;
          }
        if(excl) {
          ...
          for(const char * const *m=excl; *m; m++) {
            ...
            if(n!=FNM_NOMATCH) esyslog("music: ERROR: fnmatch(2) %s: %s",*m,strerror(errno));
            }
          if(!ok) continue;
          }
        }
    ...
    esyslog("music: ERROR: opendir %s: %s",dir,strerror(errno));
    res=false;
    }
        ...
        while(1) {
          ...
          if(!realpath(buff,real)) {
            if(errno!=ENOENT && errno!=ENOTDIR)
              esyslog("music: ERROR: realpath: %s: %s",buff,strerror(errno));
            p=Path+1;
            break;
            }
          ...
          if(!r) {
            esyslog("music: ERROR: can't find source basedir in '%s'. Outside source?",Path);
            p=Path+1;
            break;
            }
...
#ifdef DEBUG
  if(include) {
    printf("music: data: filesource %s includes (count=%d):",basedir,incCount);
    for(int i=0; i<incCount; i++) printf(" '%s'",include[i]);
    printf("\n");
    }
  else
    printf("music: data: filesource %s has no includes set\n",basedir);
#endif
  needsmount=NeedsMount;

  realbasedir=MALLOC(char,PATH_MAX+1);
  if(realpath(basedir,realbasedir)) {
    if(strcmp(basedir,realbasedir)) { esyslog("music: WARNING: source base %s expands to %s",basedir,realbasedir); }
    }
  else {
    switch(errno) {
      case EACCES:  esyslog("music: ERROR: source base %s permission denied",basedir); break;
      case ENOENT:  esyslog("music: ERROR: source base %s not found",basedir); break;
      case ENOTDIR: esyslog("music: ERROR: source base %s has invalid path",basedir); break;
      default:      esyslog("music: ERROR: source base %s realpath: %s",basedir,strerror(errno)); break;
      }
...
bool cFileSource::Parse(char *s)
{
  ...
  if((n=sscanf(s,"%255[^;];%255[^;];%d;%255[^;]",base,des,&needsmount,incl))>=3) {
    ...
    int l=strlen(base2);
    while(l>0 && base2[l-1]=='/') {
      esyslog("music: WARNING: removing trailing '/' from base %s",base2);
      base2[l-1]=0;
      l--;
      }
    ...
    if(access(realbasedir,R_OK)) { esyslog("music: WARNING: source base %s not found/permission denied",realbasedir); }
    else {
      struct stat64 ds;
      if(lstat64(realbasedir,&ds)) { esyslog("music: WARNING: can't stat source base %s",realbasedir); }
      else if(!S_ISDIR(ds.st_mode)) { esyslog("music: WARNING: source base %s is not a directory",realbasedir); }
      }
    return true;
    }
  return false;
