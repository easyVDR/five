...
bool cResample::SetInputRate(unsigned int oldrate, unsigned int newrate)
{
  if(oldrate<8000 || oldrate>newrate*6) { // out of range
    esyslog("music: WARNING: samplerate %d out of range 8000-%d\n",oldrate,newrate*6);
    return 0;
    }
  ...
  if(oldratio!=ratio) {
    printf("music: mad: new resample ratio %f (from %d kHz to %d kHz)\n",mad_f_todouble(ratio),oldrate,newrate);
    oldratio=ratio;
    }
#endif
...
double cLevel::GetLevel(void)
{
  ...
  d(printf("music: norm: new volumen level=%f peak=%f\n",level,mad_f_todouble(peak)))
  return level;
}
...
cNormalize::cNormalize(void)
{
  ...
  d(printf("music: norm: lim_lev=%f lim_acc=%d\n",d_limlvl,LIM_ACC))
  ...
  if(table) {
    d(printf("music: norm: table size=%d start=%08x jump=%08x\n",tablesize,start,F_LIM_JMP))
    ...
    fclose(out);
#endif
    d(printf("music: norm: accuracy %.12f\n",mad_f_todouble(maxdiff)))
    if(mad_f_todouble(maxdiff)>1e-6) {
      esyslog("music: ERROR: accuracy check failed, normalizer disabled");
      delete table; table=0;
      }
    }
  else esyslog("music: ERROR: no memory for lookup table, normalizer disabled");
...
#ifdef DEBUG
  printf("music: norm: gain=%f dogain=%d dolimit=%d (target=%f level=%f peak=%f)\n",dgain,dogain,dolimit,Target,Level,Peak);
...
#ifdef DEBUG
  if(total)
    printf("music: norm: stats tot=%ld lim=%ld/%.3f%% clip=%ld/%.3f%% peak=%.3f\n",
...
// the values in the lookup table. Relays heavly on libmads fixed point format.
  ...
  if(x>limlvl) {
    ...
    if(x>MAD_F_ONE) clipped++;
    limited++;
    if(i>=tablesize) printf("music: norm: overflow x=%f x-ts=%f i=%d tsize=%d\n",
...
{
#ifdef DEBUG
  printf("music: scale stats clipped=%ld peak_clip=%f peak=%f\n",
...
#ifdef DEBUG
  printf("music: shuffle: order (%d , %d -> %d) ",num,curr,in);
...
#ifdef DEBUG
    printf("music: shuffle: goto order (%d -> %d , %d -> %d) ",pos,g,curr,i);
    for(int i=0 ; i<max ; i++) printf("%d ",shuffle[i]);
    printf("\n");
#endif
    return pos;
...
void cPlayManager::ThrottleWait(void)
{
  while(!stopscan && !release && throttle) {
    db(printf("music: mgr: background scan throttled\n"))
    bgCond.Wait(listMutex);
    db(printf("music: mgr: background scan throttle wakeup\n"))
    }
}

void cPlayManager::Action(void)
{
  db(printf("music: mgr: background scan thread started (pid=%d)\n", getpid()))
  nice(5);
  listMutex.Lock();
  while(!stopscan) {
...
//      ThrottleWait();
      ...
      if(!(scan->user & SCANNED_ID3)) {
        scanning = true;
        db(printf("music: mgr: scanning (id3) %s\n",scan->Name()))
...
// TEST IT
            ...
            if(!dec->IsStream() && (!si || si->Level<=0.0) && dec->Start()) {
              db(printf("music: mgr: scanning (lvl) %s\n",scan->Name()))
              cLevel level;
              level.Init();
              bool go=true;
              while(go && !release) {
...
// TEST IT
    ...
    do {
      ...
      db(printf("music: mgr: background scan idle\n"))
      scanning = false;
      bgCond.Wait(listMutex);
      db(printf("music: mgr: background scan idle wakeup\n"))
      scanning = true;
    } while(!stopscan && (release || throttle));
  }
  listMutex.Unlock();
  db(printf("music: mgr: background scan thread ended (pid=%d)\n", getpid()))
//TEST IT  
  scanning=false;
}
  ...
  if( FILE *f = fopen ( pl_filename , "w")) {
    ...
    if(full) {
      for(cSong *song=list.First(); song; song=list.Next(song)) {
        asprintf(&buffer, "%s\n", song->Fullname());
        fprintf(f, buffer);
        d(printf("music: mgr: add track to %s: %s",pl_filename, buffer));
	free(buffer);
        }
      }
    else {
...
//    if(Active()) Stop();
  ...
  if(sidx<0) { sidx=currIndex; ns=curr; }
    ...
    if(SaveList(buff, false)) {
      isyslog("music: ctrl: refreshed playlist to file: %s\n", buff);
      }
    else
      esyslog("music: ctrl: couldn't save current playlist to file");
    free(buff);
  }      
}
...
{
  if(MP3Setup.BgrScan) {
    if(!thr && throttle) {      db(printf("music: mgr: bgr-scan -> run (%d)\n",time_ms()))
      listMutex.Lock();
      throttle=false; bgCond.Broadcast();
      listMutex.Unlock();
      }
    if(thr && !throttle) {
      db(printf("music: mgr: bgr-scan -> throttle (%d)\n",time_ms()))
      throttle=true;
      }
    }
}
...
void cPlayManager::ToggleShuffle(void)
{
  ...
  d(printf("music: mgr: shuffle mode toggled : %d\n",shuffleMode))
  if(shuffleMode && !eol) {
    curr=0; currIndex=-1;
...
{
  loopMode=!loopMode;
  d(printf("music: mgr: loop mode toggled : %d\n",loopMode))
}
...
void cPlayManager::Add(cPlayList *pl)
{
  ...
  d(printf("music: mgr: Add1: maxIndex: %d , currIndex-> %d\n",maxIndex, currIndex))
  ...
  if(real) {
    ...
    d(printf("music: mgr: Add2: maxIndex: %d , currIndex-> %d\n",maxIndex, currIndex))
    ...
    if(SaveList(buf, false)) {
      ...
      esyslog("music: ctrl: couldnt save current playlist to file");
    free(buf);  
}
...
void cPlayManager::NoScan(cSong *nono)
{
  ...
  while((nono && pass2 && scan==nono) || (!nono && scan)) {
    ...
    d(printf("music: mgr: waiting for bgr release ... (pass2=%d nono=%p scan=%p)\n",pass2,nono,scan))
...
bool cPlayManager::Next(void)
{
  ...
  if(n) {
    ...
    d(printf("music: mgr: Next(): maxIndex: %d , currIndex: %d ,  currindex: %d\n",maxIndex, currIndex, currindex))
...
bool cPlayManager::Prev(void)
{
  ...
  if(n) {
    curr=n; currIndex=ni;
    playNew=true; eol=false;
    d(printf("music: mgr: prev Track-> %d , Playlistindex-> %d\n",currIndex+1, currIndex))
    return true;
  }
...
void cPlayManager::Goto(int num)
{
  ...
  if(num>0 && num<=maxIndex+1) {
    ...
    playNew=true; eol=false;
    d(printf("music: mgr: goto Track-> %d , Playlistindex-> %d\n",currIndex + 1, currIndex))
  }
}
...
cSong *cPlayManager::Current(void)
{
  ...
  if(!play) {
    ...
    if(play){
      d(printf("music: mgr: playing %s\n",play->Name()))
      }
    else d(printf("music: mgr: nothing to play\n"))
...
#endif
  d(printf("music: dvb: using DVB output\n"))
}
...
int cOutputDvb::Output(const unsigned char *Data, int Len, bool SOF)
{
  ...
  if(SOF) {
    ...
    if(fh->samplerate!=outSr) {
      ...
      outSr=fh->samplerate;
      d(printf("music: dvb: output samplerate now %d\n",outSr))
      }
#endif
    n=FHS;
    Data+=n; Len-=n;
...
{
  fd=-1; outSr=0;
  d(printf("music: oss: using OSS output\n"))
}
  ...
  if(fd>=0) {
    ...
    if(format==AFMT_S16_LE) {
      ...
      if(channels==2) {
        ...
        d(printf("music: oss: DSP samplerate now %d\n",real))
        if(abs(real-sr)<sr/50) {
          outSr=sr;
          d(printf("music: oss: DSP reset done\n"))
          return true;
          }
        else {
          d(printf("music: oss: driver can't handle samplerate %d, got %d\n",sr,real))
          esyslog("music: ERROR: OSS driver can't handle samplerate %d, got %d\n",sr,real);
          }
        }
      else {
        d(printf("music: oss: 2-channel stereo not supported\n"))
        esyslog("music: ERROR: OSS driver doesn't support 2-channel stereo.");
        }
      }
    else {
      d(printf("music: oss: little-endian samples not supported\n"))
      esyslog("music: ERROR: OSS driver doesn't support 16-bit little-endian samples.");
      }
    close(fd); fd=-1;
    }
  return false;
...
private:
  ...
  bool Reset(unsigned int sr);
  int Alsa_Set_Buffer(int *fragment_size, int *fragment_count, int *channels);

public:
  cOutputAlsa(cMP3Player *Player);
  virtual ~cOutputAlsa();
  virtual void Init(void);
  virtual unsigned int SampleRate(unsigned int PcmSampleRate);
  virtual cFrame *MakeFrame(unsigned int & Samples, const mad_fixed_t **Data, int index, int sr);
...
cOutputAlsa::cOutputAlsa(cMP3Player *Player)
:cOutput(Player)
{
  sound_handle = NULL;
  ...
  d(printf("music: output: using ALSA output\n"))
}
...
void cOutputAlsa::Init(void)
{
    ...
    if (!sound_handle)
    {
        if ((err = snd_pcm_open (&sound_handle, alsadevice, stream, 0)) < 0) {
		esyslog ("music: output: ERROR: cannot open audio device %s (%s)\n", 
		 alsadevice,
		 snd_strerror (err));
            }
        else {
		if(fcntl(err, F_SETFL,0)!=0) {
		  poll.Add(err,true);
		}
		d(printf("music: output: Init(): opened device %s\n", alsadevice))

		err = snd_output_stdio_attach(&errlog, stderr , 0);
		if (err < 0) {
			d(printf("music: output: Init(): ERROR: snd_output_stdio_attach: %s\n", snd_strerror(err)))
		}
	}
...
int cOutputAlsa::Xrun_recovery(snd_pcm_t *handle, int err)
{
  if (err == -EPIPE) { //underrun
    err = snd_pcm_prepare(handle);
    if (err < 0)
      printf("Cant recovery from underrun, prepare failed: %s\n", snd_strerror(err));
    return 0;
    }
    ...
    if(err < 0) {
      err = snd_pcm_prepare(handle);
      if (err < 0)
        printf("Cant recovery from suspend , prepare failed: %s\n", snd_strerror(err));
    }
    return 0;
  }
  return err;
...
cFrame *cOutputAlsa::MakeFrame(unsigned int & Samples, const mad_fixed_t **Data, int index, int sr)
{
  struct FrameHeader *fh=(struct FrameHeader *)buff;
  fh->samplerate=sr;
  cFrame *f=0;
...
/*
int cOutputAlsa::Output(const unsigned char *Data, int Len, bool SOF )
{
...
int cOutputAlsa::Output(const unsigned char *Data, int Len, bool SOF )
{
  ...
  if(sound_handle) {
    ...
    while (len >0) {
      ...
      if ( (err == -EAGAIN) || (err == 0)) usleep (1000);

      if ( err < 0 ) {
        printf("music: player-mp3.c : error occured\n");
      ...
      Data += err * sample_size;
    }

    return (err>0 ? err*4 : -1);
  }
...
int cOutputAlsa::Alsa_Set_Buffer(int *fragment_size, int *fragment_count, int *channels)
{
    int err;
    unsigned int val;
    ...
    // Sampling rate
    val = output_rate;
    err = snd_pcm_hw_params_set_rate_near(sound_handle, hwparams, &val, 0);
    if (err < 0) {
      // Try 48kHZ
      printf("%d HZ not available , trying 48000 HZ\n", output_rate);
      val = 48000;
      err = snd_pcm_hw_params_set_rate_near(sound_handle,
                                       hwparams, &val, 0);
    ...
    // Two channels (stereo)
    snd_pcm_hw_params_set_channels(sound_handle, hwparams, *channels);


    periodsize = (*fragment_size) / 4;
    err = snd_pcm_hw_params_set_period_size_near(sound_handle, hwparams, &periodsize, 0);
    if (err < 0) {
    ...
    val = *fragment_count;
    err = snd_pcm_hw_params_set_periods_near(sound_handle, hwparams, &val, 0);
    if(err > 0) {
    ...
    if (err < 0) {
      esyslog("music: output: unable to set hw parameters: %s\n", snd_strerror(err));
...
//    bytes_per_frame = snd_pcm_frames_to_bytes(handle, 1);
    ...
    *fragment_count = frag_count;
    outSr = output_rate;
    d(printf("music: output: DSP(alsa) reset done\n"))
    return 1;
...
{
  return (sound_handle) ? poll.Poll(500) : false;
}

void cOutputAlsa::Play(void)
{
    if(sound_handle) snd_pcm_prepare(sound_handle);
}

void cOutputAlsa::Pause(void)
{
    if (sound_handle) snd_pcm_drain(sound_handle);
}
...
{
  if(On) {
    d(printf("music: player: player active true requested...\n"))
    if(!started) {
    ...
    d(printf("music: player: player active true done\n"))
    }
  else if(started && active) {
    d(printf("music: player: player active false requested...\n"))
    Lock();
    StopPlay();
    Unlock();
    active=false;
    ...
    Cancel(2);
    d(printf("music: player: player active false done\n"))
    }
}
...
void cMP3Player::SetPlayMode(ePlayMode mode)
{
  ...
  if(mode!=playMode) {
    playMode=mode;
    dm(printf("music: player: setting mode=%d (pid=%d)\n",mode,getpid()))
...
void cMP3Player::WaitPlayMode(ePlayMode mode, bool inv)
{
  ...
  while(active && ((!inv && mode!=playMode) || (inv && mode==playMode))) {
    dm(printf("music: player: entering wait for mode%s%d with mode=%d (pid=%d)\n",inv?"!=":"==",mode,playMode,getpid()))
    playModeCond.Wait(playModeMutex);
    dm(printf("music: player: returning from wait with mode=%d (pid=%d)\n",playMode,getpid()))
    }
}
...
void cMP3Player::Action(void)
{
  ...
  dsyslog("music: player: player thread started (pid=%d)", getpid());
  state=msStop;
  ...
  if(MP3Setup.AudioOutMode==AUDIOOUTMODE_ALSA) output=new cOutputAlsa(this);
    else
  output=new cOutputDvb(this);

  if(!output) {
    d(printf("music: player: audiooutmode mismatch or no output driver\n"))
    esyslog("music: player: ERROR: no audio output driver. balling out");
    goto abort;
  }
  ...
  while(active) {
    ...
    if(now>=beat) {
      ...
      printf("music: player: heartbeat buffer=%d now=%d\n",avail,now&4095);
...
next:
    ...
    if(pframe) {
      ...
      if(lastwrite && lastwrite<now-(DEBUG_DELAY+50))
        printf("music: player: write delayed %d ms\n",now-lastwrite);
      lastwrite=now;
      }
#endif
...
// Spectrum Analyzer: Tell SpAn which timestamp is currently playing
        ...
        LOG_ERROR;
        d(printf("music: player: output failed: %s\n",strerror(errno)))
        Unlock();
        goto abort;
        }
      }
    ...
    if(mgr->NewCurrent() && playMode==pmPlay && state!=msStart) {
      Empty();
      state=msRestart;
      d(printf("music: player: stale song change, restart.\n"))
    }

    if(!rframe && playMode==pmPlay) {
      switch(state) {
        case msStart:
          d(printf("music: player: starting play\n"))
          mgr->Throttle(true);
...
// check for existing file
          if(playing) {
            if((decoder=playing->Decoder()) && decoder->Start()) {
              ...
              if(si) {
                if(si->Level>0.0) {
                  d(printf("music: player: found song level=%f peak=%f\n",si->Level,si->Peak))
                  haslevel=true;
              ...
              d(printf("music: player: isStream=%d levelgood=%d haslevel=%d\n",isStream,levelgood,haslevel))
              output->Init();
              level.Init();
              state=msDecode;
              ...
              esyslog("music: player: ERROR: playlist entry %s is not a valid file",playing->Name());
			      }
          }
          else
            d(printf("music: player: no current on start play\n"))
          state=msEof;
          break;
        case msDecode:
          {
...
#ifdef DEBUG_DELAY
          now=time_ms()-now;
          if(now>DEBUG_DELAY) printf("music: player: decode delayed %d ms\n",now);
#endif
          switch(ds->status) {
            case dsPlay:
              pcm=ds->pcm;
        ...
        case msResample:
          ...
          if(oldrate!=pcm->samplerate) {
            printf("music: player: new input sample rate %d\n",pcm->samplerate);
        ...
        case msError:
        case msEof:
          d(printf("music: player: eof or error\n"))
          state=msWait;
          d(printf("music: player: state=msEof\n"))
          // fall through

        case msRestart:
        case msStop:
          d(printf("music: player: stopping play\n"))
          ...
          output->Stats(); if(haslevel) norm.Stats();
#endif
          d(printf("music: player: state=msStop\n"))
        ...
        case msWait:
          if(ringBuffer->Available()==0) {
            if(mgr->NextCurrent()) {
              d(printf("music: player: playing next\n"))
              state=msStart;
              }
            else {
              d(printf("music: player: end of playlist\n"))
...
abort:
  ...
  dsyslog("music: player: player thread ended (pid=%d)", getpid());
}

void cMP3Player::Empty(void)
{
  if(MP3Setup.AudioOutMode!=AUDIOOUTMODE_ALSA)  Lock(); // need fix
  delete rframe;
  rframe=0;
  pframe=0;
  ringBuffer->Clear();
  DeviceClear();
  if(MP3Setup.AudioOutMode!=AUDIOOUTMODE_ALSA)  Unlock(); // need fix
}
...
bool cMP3Player::PrevCheck(void)
{
  ...
  if(playindex>=2000 && !isStream) {
    ...
    Empty();
    d(printf("music: player: skip to start of song\n"))
    }
  Unlock();
  return res;
}
...
void cMP3Player::SkipSeconds(int secs)
{
  if(playMode!=pmStopped && !isStream) {
    ...
    if(secs>0 && bufsecs>=(float)secs) {
      ...
      d(printf("music: player: skipping play=%d skip=%d ...\n",playindex,skipindex))
      cFrame *f;
      ...
      d(printf("music: player: skipped play=%d skip=%d\n",playindex,skipindex))
      }
    else {
...
bool cMP3Player::ExistsLyrics(const char *songpath)
{
  ...
  if(pos!=std::string::npos) {
    ...
    filename += ".lyrics";
#ifdef DEBUG_COVER
    printf("music: player: check for lyrics '%s'\n", filename.c_str());
#endif
    if( FILE *f = fopen(filename.c_str(), "r") ) {
#ifdef DEBUG_COVER
	printf("music: player: found lyrics '%s'\n", filename.c_str());
#endif
	LyricsFound = true;
      fclose(f);
      }
...
#ifdef DEBUG_COVER
      printf("music: player: no lyrics found\n");
#endif
      LyricsFound = false;
    }
  }      
