...
int RunCommand(const char *cmd, const char *State, const char *Name=0)
{
  ...
  if(cmd) {
    ...
    d(printf("music: net: executing '%s'\n",tmp))
    res=SystemExec(tmp);
    free(tmp);
    }
  return res;
...
{
  d(printf("music: net: netscript down delay\n"))
...
{
  d(printf("music: net: name lookup %s\n",hostname))
  ...
  if(hp) {
    ...
    d(printf("music: net: connecting to %s:%d\n",hostname,port))
    if(connect(fd,(struct sockaddr *)&sin,sizeof(sin))==0) {
      d(printf("music: net: connected\n"))
      Done(1);
      }
    else { esyslog("music: connect() failed: %s",strerror(errno)); Done(-1); }
    }
  else { esyslog("music: Unknown host '%s'",hostname); Done(-1); }
}
...
bool cNet::Connect(const char *hostname, const int port)
{
  ...
  if(fd>=0) {
    ...
    if(res>0) {
      ...
      else esyslog("music: fnctl() failed: %s",strerror(errno));
      }
    else if(res==0) esyslog("music: Connection timed out");
    }
  else esyslog("music: socket() failed: %s",strerror(errno));
  Disconnect();
  return false;
}
...
int cNet::Write(unsigned char *dest, int len)
{
  ...
  do {
    if(poll.Poll(rwTimeout)) {
      r=write(fd,dest,len);
      if(r<0 && errno!=EAGAIN) {
        esyslog("music: write() failed: %s",strerror(errno));
        break;
        }
      dest+=r; len-=r; t+=r;
      }
    else { esyslog("music: Write timed out"); break; }
    } while(len>0);
  return t;
}
...
int cNet::RingRead(unsigned char *dest, int len)
{
  ...
  for(;;) {
    if(!Available() && deferedErrno) {
      d(printf("music: net: ringbuffer empty, async read bailed out\n"))
      return -1;
      }
    rd=Get(r);
    if(rd && r>0) {
...
{
  d(printf("music: net: async read started\n"))
  ...
  while(connected) {
    if(poll.Poll(rwTimeout)) {
      ...
      if(r>0) {
        ...
        deferedErrno=errno;
        esyslog("music: read() failed: %s",strerror(errno));
        break;
        }
      else if(r==0) {
        deferedErrno=-1;
        d(printf("music: net: EOF from read()\n"))
        break;
        }
      }
    else {
      deferedErrno=-1;
      esyslog("music: Read timed out");
      break;
      }
    }
  EnableGet();
