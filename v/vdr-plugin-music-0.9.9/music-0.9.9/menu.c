...
{
// TODO: Place to check for added Tracks
  using namespace std;
  ifstream filestr;
  std::string Datei;
  std::string line;
...
//  printf("Datei=%s\n\n\n",Datei.c_str());
  ...
  if(item->Type()==otFile) {
    ...
    if(MP3Setup.PlayerActive) {
      filestr.open(Datei.c_str());
      if(filestr) {
        while (getline(filestr, line, '\n')) {
          string::size_type pos = line.find ( item->Path(), 0);
          if(pos != string::npos) {
            checked = true;
            break;
            }
    ...
    if(checked)
      asprintf(&buffer, "%s\t %s\t  %s", Icons::Menu_Note(), Icons::Menu_CheckedBox(), item->Name());
    else
      asprintf(&buffer, "%s\t %s\t  %s", Icons::Menu_Note(), Icons::Menu_Checkbox(), item->Name());
...
/*
  if(item->Type()==otFile) {
    if( (fs=fopen(Datei.c_str(), "r")) ) {
      printf("Datei=%s\n",Datei.c_str());
      while( fgets(line, sizeof(line), fs) != NULL) {

        printf("line = %s , item->Path()= %s \n", line, item->Path()); 

        if(strstr(line, item->Path()))
          asprintf(&buffer, "%s\t %s\t  %s", Icons::Menu_Note(), Icons::Menu_CheckedBox(), item->Name());
        else
          asprintf(&buffer, "%s\t %s\t  %s", Icons::Menu_Note(), Icons::Menu_Checkbox(), item->Name());
        }
...
*/
    ...
    asprintf(&buffer, "%s\t %s\t  %s", Icons::Menu_Folder(), " ", item->Name());
  ...
  if(state==osUnknown) {
    switch (Key) {
      ...
      case kBlue:   if(withID3) state=ID3Info();
                    break;
//      case kYellow: printf("PRESS YELLOW(menubrowse: 1111\n");	break;    
