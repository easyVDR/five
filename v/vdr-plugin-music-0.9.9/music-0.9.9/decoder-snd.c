...
#ifdef GUARD_DEBUG
  if(framebuff) {
    printf("music: decoder-snd: bufferguard");
...
bool cSndDecoder::Start(void)
{
  ...
  if(file.Open() && info.DoScan(true)) {
    d(printf("music: decoder-snd: open rate=%d frames=%lld channels=%d format=0x%x seek=%d\n",
    ...
    else esyslog("music: ERROR: cannot play sound file %s: more than 2 channels",filename);
    }
  cDecoder::Unlock();
  return Clean();
}
...
bool cSndDecoder::Skip(int Seconds, float bsecs)
{
  ...
  if(playing && file.sfi.seekable) {
    ...
    d(printf("music: decoder-snd: skip: secs=%d fsecs=%f frames=%lld current=%lld new=%lld\n",Seconds,fsecs,frames,file.Seek(0,true),newpos))
    ...
    if(frames>=0) {
      ...
      printf("music: decoder-snd: skipping to %02d:%02d (frame %lld)\n",i/60,i%60,frames);
#endif
      res=true;
      }
    }
    ...
    while(fgets(line,sizeof(line),f)) {
      if(!state) {
        ...
        if(sscanf(line,CDFS_MARK_ID,&id,&n)==2) {
          d(printf("music: decoder-snd: discid: found id=%08x n=%d\n",id,n))
        ...
        if(sscanf(line,CDFS_MARK_TR,&off,&end)==2) {
          dc(printf("music: decoder-snd: discid: found offset=%d end=%d for track %d\n",off,end,tr+1))
          offsets[tr]=off+CDFS_TRACK_OFF;
          if(++tr==ntrks) {
            nsecs=(end+1)/75;
            dc(printf("music: decoder-snd: discid: nsecs=%d / 0x%x\n",nsecs,nsecs))
            res=true;
            break;
            }
          }
...
{
  Title=Artist=Genre=0; TTitle=ExtT=0;
}
...
{
  Album=Artist=Genre=0; DTitle=ExtD=0; DiscID=0;
}
...
bool cCDDBDisc::TrackInfo(int tr, cSongInfo *si)
{
  ...
  if(s) {
    ...
    Put(Album,si->Album);
    if(s->Genre) Put(s->Genre, si->Genre);
...
bool cCDDBDisc::Load(cDiscID *id, const char *filename)
{
  ...
  d(printf("music: decoder-snd: cddb: loading discid %08x from %s\n",id->discid,filename))
  DiscID=id->discid;
  ...
  if(f) {
        ...
        if(p) {
           *p=0;
           ...
           if(*name && *value) {
             ...
             else if(!strcasecmp(name,"DGENRE")) Strcat(Genre,value);
    ...
    if(Count()>0) {
      ...
      d(printf("music: decoder-snd: cddb: found artist='%s' album='%s' genre='%s' isSampler=%d\n",Artist,Album,Genre,isSampler))
      free(DTitle); DTitle=0;

      if(!isSampler && Artist && Album && !strncmp(Album,Artist,strlen(Artist))) {
        d(printf("music: decoder-snd: cddb: detecting sampler from Artist==Album\n"))
        isSampler=true;
        }
      ...
      if(!isSampler) {
        ...
        if(nofail1==Count() || nofail2==Count()) {
          d(printf("music: decoder-snd: cddb: detecting sampler from nofail\n"))
          isSampler=true;
          }
        }
      ...
      free(ExtD); ExtD=0;
      d(printf("music: decoder-snd: cddb: found year=%d\n",Year))
      ...
      while(s) {
        ...
        d(printf("music: decoder-snd: cddb: found track %d title='%s' artist='%s'\n",s->Track,s->Title,s->Artist))
        s=Next(s);
        }
      return true;
      }
...
bool cCDDB::RemoteGet(cDiscID *id)
{
  ...
  if(net->Connect(MP3Setup.CddbHost,MP3Setup.CddbPort)) {
    ...
    if(code/100==2) {
      ...
      code=DoCddbCmd("cddb hello %s %s %s %s\n",user,host,"vdr-plugin-music", "0.3.0-rc2");
      if(code/100==2) {
            ...
            if(cat) {
              ...
              if(code==210) {
                ...
                if(MakeDirs(name,false)) {
                  ...
                  else esyslog("music: fopen() failed: %s",strerror(errno));
                  }
                free(name);
                }
              else if(code>0) esyslog("music: server read error: %d %s",code,cddbstr);
              free(cat);
              }
            }
          else if(code>0) esyslog("music: server query error: %d %s",code,cddbstr);
          }
        else esyslog("music: server proto error: %d %s",code,cddbstr);
        }
      else if(code>0) esyslog("music: server hello error: %d %s",code,cddbstr);
      }
    else if(code>0) esyslog("music: server sign-on error: %d %s",code,cddbstr);
    }

  delete net; net=0;
  MP3Setup.ShowStatus = 0;
  return res;
}
...
#ifdef CDDB_DEBUG
    if(log) printf("music: decoder-snd: cddb: <- %s",buff);
#endif
    return true;
    }
  return false;
...
int cCDDB::GetCddbResponse(void)
{
  ...
  if(GetLine(buf,sizeof(buf))) {
    ...
    else esyslog("music: Unexpected server response: %s",buf);
    }
  return -1;
}
...
int cCDDB::DoCddbCmd(const char *format, ...)
{
  ...
  va_end(ap);
#ifdef CDDB_DEBUG
  printf("music: decoder-snd: cddb: -> %s",buff);
#endif
  int r=net->Puts(buff);
  free(buff);
  if(r<0) return -1;
...
//  TITLE   = sf_get_string(file->sf, SF_STR_TITLE);
//  ARTIST  = sf_get_string(file->sf, SF_STR_ARTIST);
//  COMMENT = sf_get_string(file->sf, SF_STR_COMMENT);
//  YEAR    = sf_get_string(file->sf, SF_STR_DATE);


//  printf("FLAC-TITLE   = '%s'\n", TITLE.c_str());
//  printf("FLAC-ARTIST  = '%s'\n", ARTIST.c_str());
//  printf("FLAC-COMMENT = '%s'\n", COMMENT.c_str());
//  printf("FLAC-YEAR    = '%s'\n", YEAR.c_str());

  if(sf_get_string(file->sf, SF_STR_TITLE))   printf("FLAC-TITLE   = '%s'\n", sf_get_string(file->sf, SF_STR_TITLE));
  if(sf_get_string(file->sf, SF_STR_ARTIST))  printf("FLAC-ARTIST  = '%s'\n", sf_get_string(file->sf, SF_STR_ARTIST));
  if(sf_get_string(file->sf, SF_STR_ALBUM))   printf("FLAC-ALBUM   = '%s'\n", sf_get_string(file->sf, SF_STR_ALBUM));
  if(sf_get_string(file->sf, SF_STR_COMMENT)) printf("FLAC-COMMENT = '%s'\n", sf_get_string(file->sf, SF_STR_COMMENT));
  if(sf_get_string(file->sf, SF_STR_DATE))    printf("FLAC-YEAR    = '%s'\n", sf_get_string(file->sf, SF_STR_DATE));

  if(sf_get_string(file->sf, SF_STR_TITLE))   Title   = strdup(sf_get_string(file->sf, SF_STR_TITLE)); else  FakeTitle(file->Filename);
  if(sf_get_string(file->sf, SF_STR_ARTIST))  Artist  = strdup(sf_get_string(file->sf, SF_STR_ARTIST));
  if(sf_get_string(file->sf, SF_STR_ALBUM))   Album   = strdup(sf_get_string(file->sf, SF_STR_ALBUM));
  if(sf_get_string(file->sf, SF_STR_COMMENT)) Comment = strdup(sf_get_string(file->sf, SF_STR_COMMENT));
  if(sf_get_string(file->sf, SF_STR_DATE))    Year    = atoi(sf_get_string(file->sf, SF_STR_DATE));
...
void cSndFile::Error(const char *action)
{
  ...
  esyslog("music: ERROR: sndfile %s failed on %s: %s",action,Filename,buff);
}
