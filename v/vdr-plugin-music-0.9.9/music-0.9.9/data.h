...
class cFileObjItem : public cListObject {
private:
  char *path, *name;
  eObjType type;
public:
  cFileObjItem(const char *Path, const char *Name, const eObjType Type);
  cFileObjItem(const cFileObjItem *objfile);
  ~cFileObjItem();
...
public:
  ...
  cFileSource *FindSource(const char *filename, bool external);
};
