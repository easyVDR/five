...
void cOggFile::Error(const char *action, const int err)
{
  ...
  esyslog("music: ERROR: vorbisfile %s failed on %s: %s",action,Filename,errstr);
}
...
bool cOggInfo::DoScan(bool KeepOpen)
{
  ...
  if(!vi) Abort(false);
  d(printf("music: decoder-ogg: info ch=%d srate=%ld brate_low=%ld brate_high=%ld brate_avg=%ld\n",
...
bool cOggDecoder::Start(void)
{
  ...
  if(file.Open() && info.DoScan(true)) {
    d(printf("music: decoder-ogg: open rate=%d channels=%d seek=%d\n",
             info.SampleFreq,info.Channels,file.CanSeek()))
    if(info.Channels<=2) {
      Unlock();
      return true;
      }
    else esyslog("music: ERROR: cannot play ogg file %s: more than 2 channels",filename);
    }
  Clean();
  Unlock();
  return false;
...
bool cOggDecoder::Skip(int Seconds, float bsecs)
{
  ...
  if(playing && file.CanSeek()) {
    ...
    d(printf("music: decoder-ogg: skip: secs=%d fsecs=%f current=%lld new=%lld\n",Seconds,fsecs,file.IndexMs(),newpos))
...
#ifdef DEBUG
      int i=index/1000;
      printf("music: decoder-ogg: skipping to %02d:%02d\n",i/60,i%60);
#endif
      res=true;
      }
    }
