...
#ifdef DEBUG
  isyslog("music: id3tag: loading year.dat from '%s'",Datei.c_str());
#endif
...
#ifdef DEBUG
  isyslog("music: id3tag: loading genre_set.dat from '%s'",Datei.c_str());
#endif
...
:cOsdMenu(tr("Music: ID3tag editor"),20)
{
  iRating=-1;
  id3_Song = song;
...
void cMP3id3Tag::ShowDisplay(void)
{
  ...
  Add(new cMenuEditStrItem(tr("Genre")           ,Genre       , sizeof(Genre)));
...
//  Datei = Datei + "/scripts/music_id3v2";
...
#ifdef DEBUG
  isyslog("music: id3tag: executing 'id3v2'");
#endif
  ...
  if( FILE* fd=fopen(id3_Song->Fullname(), "r")) {
    ...
    if(strcmp(cDecoders::ID2Str(id3_songinfo->DecoderID),"MP3")) {
      esyslog("music: id3tag: No valid MP3 file");
      }
    else {
      char *cmd;
      asprintf(&cmd, "%s --delete-all '%s'", Datei.c_str(), id3_Song->Fullname());
#ifdef DEBUG
      dsyslog("music: id3tag: RemoveTagsOnly(): executing '%s'", cmd);
#endif
      system(cmd);
      free(cmd);
    }  
    ...
    esyslog("music: id3tag: RemoveTagsOnly(): ERROR while removing id3tags for '%s'", id3_Song->Fullname());
  }
...
//  Datei = Datei + "/scripts/music_id3v2";
...
#ifdef DEBUG
  dsyslog("music: id3tag: executing id3v2");
#endif
  ...
  if(MP3Setup.RateInFile) {
    if( FILE* fd=fopen(id3_Song->Fullname(), "r")) {
      ...
      if(strcmp(cDecoders::ID2Str(id3_songinfo->DecoderID),"MP3")) {
        isyslog("music: id3tag: No valid MP3 file");
        }
      else {
        ...
        char *cmd;
        asprintf(&cmd, TAG_CMD, Datei.c_str(), artist.c_str(), album.c_str(), title.c_str(), genre.c_str(), genre.c_str(), MP3Setup.RateCompat, Rating, Year,file.c_str());

#ifdef DEBUG
      dsyslog("music: id3tag: SaveTags(): executing '%s'", cmd);
#endif
        system(cmd);
        free(cmd);
      ...
      esyslog("music: id3tag: ERROR while writing id3tags for '%s'", id3_Song->Fullname());
      }
    }
  else {
#ifdef DEBUG
    dsyslog("music: id3tag: SaveTags(): change id3-tag only in cache");
#endif
    mgr->UpdateSong(Artist, Album, Title, Genre, Year, Rating, id3_Song);
//    InfoCache.Modified();
  }
...
void cMP3id3Tag::GetRealTags(void) {
  ...
  if(id3_songinfo) {
    if(id3_songinfo->HasInfo() && id3_songinfo->Total>0) {
      strncpy(FullFile ,id3_Song->Fullname(), sizeof(FullFile) );

      if(id3_songinfo->Artist != NULL)
        strncpy(Artist   ,id3_songinfo->Artist, sizeof(Artist) );
      else
        strcpy(Artist, "");

      if(id3_songinfo->Album != NULL)
        strncpy(Album    ,id3_songinfo->Album, sizeof(Album));
      else
        strcpy(Album, "");	

      if(id3_songinfo->Title != NULL)
        strncpy(Title    ,id3_songinfo->Title, sizeof(Title));
      else
        strcpy(Title, "");	

      if(id3_songinfo->Genre != NULL)
        strncpy(Genre    ,id3_songinfo->Genre, sizeof(Genre));
      else
        strcpy(Genre, "Other");	
...
void cMP3id3Tag::ShowDisplay(void)
{
  ...
  strncpy(artist, Artist.c_str(), sizeof(artist) );
  strncpy(album,  Album.c_str(), sizeof(album)  );
  strncpy(title,  Title.c_str(), sizeof(title)  );
  strncpy(file,   id3_Song->Fullname(), sizeof(file)  );
  strncpy(genre,  Genre.c_str(), sizeof(genre)  );
...
#ifdef DEBUG
  isyslog("music: id3tag: executing music_id3v2 from '%s'",Datei.c_str());
#endif
...
#ifdef DEBUG
  dsyslog("music: id3tag: executing music_id3v2 from '%s'",Datei.c_str());
#endif
  ...
  if(MP3Setup.RateInFile) {
    if( FILE* fd=fopen(id3_Song->Fullname(), "r")) {
      ...
      dsyslog("music: id3tag: SaveTags(): executing '%s'", cmd);
        ...
        Skins.Message(mtStatus, tr("Update id3-tag"));
        mgr->UpdateSong(Artist.c_str(), Album.c_str(), Title.c_str(), Genre.c_str(), Year, Rating, id3_Song);
        Skins.Message(mtStatus, NULL);
	}
      }    
    else {
...
void cMP3id3Tag::GetRealTags(void) {
  ...
  if(id3_songinfo) {
    if(id3_songinfo->HasInfo() && id3_songinfo->Total>0) {
      FullFile = id3_Song->Fullname();

      if(id3_songinfo->Artist != NULL)
        Artist = id3_songinfo->Artist;
      else
        Artist = "";

      if(id3_songinfo->Album != NULL)
        Album = id3_songinfo->Album;
      else
        Album = "";

      if(id3_songinfo->Title != NULL)
        Title = id3_songinfo->Title;
      else
        Title = "";

      if(id3_songinfo->Genre != NULL)
        Genre = id3_songinfo->Genre;
      else
        Genre = "Other";
