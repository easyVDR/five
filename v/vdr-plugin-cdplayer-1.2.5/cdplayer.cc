...
std::string cPluginCdplayer::mCDDBServer = "gnudb.gnudb.org";
std::string cPluginCdplayer::mCDDBCacheDir = "";
int cPluginCdplayer::mCDDBPort = 8880;
...
const char *cPluginCdplayer::CommandLineHelp(void)
{
    return "-d  --device  <device>    CD-Rom device : /dev/cdrom\n"
           ...
           "-S  --cddbserver <server> CDDB server name : gnudb.gnudb.org\n"
           "-P  --cddbport <port>     CDDB server port : 8880\n"
...
bool cPluginCdplayer::ProcessArgs(int argc, char *argv[])
{
    ...
    static struct option long_options[] =
    {
        ...
        { "cddbport",           required_argument, NULL, 'P'  },
    ...
    while ((c = getopt_long(argc, argv, "d:s:c:S:P:C:nN",
    ...
    case 'P':
        int port;
        if ((port = atoi(optarg)) > 0) mCDDBPort = port;
        break;
        case 'C':
