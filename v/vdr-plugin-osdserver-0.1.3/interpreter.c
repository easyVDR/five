...
class cMessageCallback : public cMainThreadLock::cCallback {
    eMessageType Type;
    const char *s;
    int Seconds;
...
int cMessageCallback::Callback() {
    return Skins.Message(Type, s, Seconds);				
}

eKeys cMessageCallback::Message(eMessageType _Type, const char *_s, int _Seconds) {
    Type = _Type;
    s = _s;
    Seconds = _Seconds;
    return (eKeys)CallCallback();
}
...
bool cCommandProcessor::CmdMessage(cCommand &cmd) {
    ...
    */
    ...
    if (queue) {
        ...
        switch (mode) {
        case status:
            k=Callback.Message(mtStatus,cmd.GetArg(0),0);
            break;
        case statusclear:
            k=Callback.Message(mtStatus,NULL,0);
            break;
        case info:
            k=Callback.Message(mtInfo,cmd.GetArg(0),seconds);
            break;
        case warn:
            k=Callback.Message(mtWarning,cmd.GetArg(0),seconds);
            break;
        case error:
            k=Callback.Message(mtError,cmd.GetArg(0),seconds);
            break;
        }
