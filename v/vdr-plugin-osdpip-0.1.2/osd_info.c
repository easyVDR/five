...
:   m_Osd(osd),
    ...
    m_Channel(NULL)
{
    m_Bitmap = new cBitmap(OsdPipSetup.InfoWidth, OsdPipSetup.ShowInfo * 35, OsdPipSetup.ColorDepth == kDepthGrey16 ? 4 : 8);
}
...
void cOsdInfoWindow::Show(bool Refresh)
{
    ...
    if (OsdPipSetup.ColorDepth == kDepthGrey16)
    {
        ...
        m_Bitmap->DrawText((cControl::Control() && cReplayControl::NowReplaying()) || m_Message ? 0 : 20, 0, channel, clrWhite, clrBlack, font, m_Bitmap->Width(), 34);
        if (m_Bitmap->Height() > 35)
        {
            if (cReplayControl::NowReplaying() && cControl::Control() && !m_Message) {
                m_Bitmap->DrawRectangle(0, 35, m_Bitmap->Width() - 1, m_Bitmap->Height() - 1, clrBlack);
                cControl::Control()->GetIndex(pos, end);
            ...
            {
                m_Bitmap->DrawText(20, 35, presentTime, clrWhite, clrTransparent, font, 90, 34);
                m_Bitmap->DrawText(100, 35, presentName, clrWhite, clrTransparent, font, m_Bitmap->Width() - 90, 34);
                if (progress != 0)
                {
        ...
        if (m_Bitmap->Height() > 2*35)
        {
            m_Bitmap->DrawText(0, 2*35, followingTime, clrWhite, clrBlack, font, 90, 34);
            m_Bitmap->DrawText(90, 2*35, followingName, clrWhite, clrBlack, font, m_Bitmap->Width() - 90, 34);
        }
    }
    else
    {
        ...
        for (int i = 0; i < 256; i++)
            m_Bitmap->SetColor(i, m_Palette[i]);
        const cFont *font = cFont::GetFont(fontOsd);
        m_Bitmap->DrawText((cControl::Control() && cReplayControl::NowReplaying()) || m_Message ? 0 : 20, 0, channel, m_Palette[255], m_Palette[0], font, m_Bitmap->Width(), 34);
        if (m_Bitmap->Height() > 35)
        {
            if (cReplayControl::NowReplaying() && cControl::Control() && !m_Message)
            {
                m_Bitmap->DrawRectangle(0, 35, m_Bitmap->Width() - 1, m_Bitmap->Height() - 1, m_Palette[0]);
                cControl::Control()->GetIndex(pos, end);
            ...
            {
                m_Bitmap->DrawText(20, 35, presentTime, m_Palette[255], m_Palette[0], font, 90, 34);
                m_Bitmap->DrawText(100, 35, presentName, m_Palette[255], m_Palette[0], font, m_Bitmap->Width() - 90, 34);
                if (progress != 0)
                {
        ...
        if (m_Bitmap->Height() > 2*35)
        {
            m_Bitmap->DrawText(20, 2*35, followingTime, m_Palette[255], m_Palette[0], font, 90, 34);
            m_Bitmap->DrawText(100, 2*35, followingName, m_Palette[255], m_Palette[0], font, m_Bitmap->Width() - 90, 34);
        }
    }
    tmp = NULL;
