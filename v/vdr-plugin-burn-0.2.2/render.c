...
namespace vdr_burn
{
	...
	bool renderer::execute_render( const Type& item, int page )
	{
		...
		render.put_environment( "MENU_ASPECT", item.get_menu_aspect() );
	...
	bool renderer::CreateTitleMenuPage(int &Lines, int Page)
	{
		...
		Lines = menuPage.draw_text( m_job.get_skin().get_title_menu_text(), convert_to_utf8( m_current->get_eventDescription() ), Lines );
	...
	bool renderer::operator()()
	{
		try {
			...
			logger::error( string( "Error while rendering: " ) + ex.what() );
			return false;
		}
