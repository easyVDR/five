...
void cSongInfo::FakeTitle(const char *filename, const char *extention)
{
  ...
  if(!Title && filename)  {
    ...
    if(s && *s=='/') {
      ...
      d(printf("music-sd: decoder: faking title '%s' from filename '%s'\n",Title,filename))
      }
    }
}
  ...
  if(Filename) {
    ...
    if(!stat64(Filename,&ds)) {
      if(S_ISREG(ds.st_mode)) {
        ...
        else if(errno!=ENOSYS && log) { esyslog("music-sd: ERROR: can't statfs %s: %s",Filename,strerror(errno)); }
        Filesize=ds.st_size;
        CTime=ds.st_ctime;
#ifdef CDFS_MAGIC
      ...
      else if(log) { esyslog("music-sd: ERROR: %s is not a regular file",Filename); }
      }
    else if(log) { esyslog("music-sd: ERROR: can't stat %s: %s",Filename,strerror(errno)); }
    }
  return false;
}
...
#endif
*/
            default:      esyslog("music-sd: ERROR: bad DecoderID '%s' from info cache: %s",cDecoders::ID2Str(dat->DecoderID),full); break;
      }
    }
    dat->Unlock();
  }
...
*/
    ...
    if(!decoder) esyslog("music-sd: ERROR: no decoder found for %s",Obj->Name());
    }
  return decoder;
}
  ...
  if(touch+CACHEPURGETIMEOUT*SECS_PER_DAY/10 < now) {
    if(!Removable()) {                            // is this a permant source?
      ...
      if(stat64(Filename,&ds) || !S_ISREG(ds.st_mode) || access(Filename,R_OK)) {
        d(printf("music-sd: decoder: cache: purged: file not found %s\n",Filename))
        return true;
        }
      }
    }
  ...
  while(fgets(buf,sizeof(buf),f)) {
    ...
    if(name) {
      ...
      if(value) {
        ...
        else d(printf("music-sd: decoder: cache: ignoring bad token '%s' from cache file\n",name))
        }
      }
    }
...
void cInfoCache::Cache(cSongInfo *info, cFileInfo *file)
{
  ...
  if(dat) {
    ...
    Modified();
    dat->Unlock();
    d(printf("music-sd: decoder: cache: updating infos for %s\n",file->Filename))
    }
  else {
    dat=new cCacheData;
    dat->Create(file,info);
    AddEntry(dat);
    d(printf("music-sd: decoder: cache: New caching infos for %s\n",file->Filename))
    }
  lock.Unlock();
}
...
void cInfoCache::Update(int rating, cFileInfo *file)
{
  ...
  while(dat) {
    if(dat->hash==hash && !strcmp(dat->Filename,file->Filename)) {
      d(printf("music-sd: decoder: Update: Updating infos for %s\n",file->Filename))
      dat->Lock();
      dat->Rating = rating;
      dat->Unlock();
    }
...
#ifdef DEBUG
  printf("music-sd: decoder: ID3 cache emptied\n");  
#endif  
}
...
//    if(dat->hash==hash && !strcmp(dat->Filename,file->Filename) && dat->Filesize==file->Filesize) {
    if(dat->hash==hash && !strcmp(dat->Filename,file->Filename)) {
      d(printf("music-sd: decoder: cache: Existing entry found: %s\n",dat->Filename))
      dat->Lock();
      ...
      if((!file->FsID && !dat->FsID) || (file->FsID && dat->FsID && !strcmp(dat->FsID,file->FsID))) {
        ...
        if(dat->CTime!=file->CTime) {
          d(printf("music-sd: decoder: cache: ctime differs, removing from cache: %s\n",dat->Filename))
          DelEntry(dat); dat=0;
          }
        break;
        }
...
bool cInfoCache::Purge(void)
{
  ...
  if(now-lastpurge>(60*60)) {
    ...
    Start();
    lastpurge=now;
//    d(printf("music-sd: cleaned up id3 cache\n"));
    }
  return Active();
}
...
{
  d(printf("music-sd: cache: id3 cache purge thread started (pid=%d)\n",getpid()))
  nice(3);
  lock.Lock();
  ...
  lock.Unlock();
  d(printf("music-sd: cache: id3 cache purge thread ended (pid=%d)\n",getpid()))
}
...
char *cInfoCache::CacheFile(void)
{
  ...
  d(printf("music-sd: decoder: cachedir = '%s'\n", cachedir.c_str()));

  // create dir and cachefile if not exist
  if(!(DirectoryOk(cachedir.c_str(),true))) {
    if(MakeDirs(cachedir.c_str(), true)) {
      isyslog("music-sd: cachedir created '%s'\n",cachedir.c_str());

      ofstream out(cachefile.c_str());
      out<<""<<endl;
      out.close();
      isyslog("music-sd: empty cachefile created '%s'\n",cachefile.c_str());
      }
    else
      esyslog("music-sd: ERROR: could not create cachedir '%s'\n", cachedir.c_str());
  }
...
//  if(MP3Setup.AutoPurge)  Purge();
  if(modified && (force || (!Purge() && time(0)>lasttime))) {
    ...
    if(f.Open()) {
      ...
      d(printf("music-sd: decoder: cache: saved cache to file\n"))
      }
    }
}
...
void cInfoCache::Load(bool reload)
{
  ...
  if(access(name,F_OK)==0) {
    if(!reload)  
      isyslog("music-sd: loading id3 cache from %s",name);
    else  
      isyslog("music-sd: reloading new id3 cache from %s",name);
    FILE *f=fopen(name,"r");
    if(f) {
      char buf[256];
      bool mod=false;
      ...
      while(fgets(buf,sizeof(buf),f)) {
        if(!strcasecmp(buf,"##BEGIN\n")) {
          ...
          if(dat->Load(f)) {
            ...
            delete dat;
            if(ferror(f)) {
              esyslog("music-sd: ERROR: failed to load id3 cache");
              break;
              }
            }
          }
...
// between ratingFrom <-> ratingTo
  if(ratingFromTo) {
    isyslog("music-sd: Search ratingFromTo");
    if( (fs=fopen(Datei.c_str(), "r")) && (ft=fopen(Outfile.c_str(), "w")) ) {
      count=0;
      while( fgets(line, sizeof(line), fs) !=NULL ) {
        word = strtok(line ,delim);
...
// check ratingFrom
    ...
    isyslog("music-sd: Search ratingFrom '%d' '%d'",szRatingFrom, y );
    if( (fs=fopen(Datei.c_str(), "r")) && (ft=fopen(Outfile.c_str(), "w")) ) {
      count=0;
      while( fgets(line, sizeof(line), fs) !=NULL ) {
        word = strtok(line ,delim);
...
// check ratingTo
    ...
    isyslog("music-sd: Search ratingTo '%d' '%d'",szRatingTo, y2 );
    if( (fs=fopen(Datei.c_str(), "r")) && (ft=fopen(Outfile.c_str(), "w")) ) {
      count=0;
      while( fgets(line, sizeof(line), fs) !=NULL ) {
        word = strtok(line ,delim);
...
// dont check for rating
    ...
    isyslog("music-sd: Search not for rating '%d' '%d'",szRatingFrom, szRatingTo );
    if( (fs = fopen(Datei.c_str(),"r")) && (ft=fopen(Outfile.c_str(),"w")) ) {
      count=0;
      while( fgets(line, sizeof(line),fs) != NULL ) {
        count++;
