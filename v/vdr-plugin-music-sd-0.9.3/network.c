...
//#define CON_TIMEOUT      10*1000   // default timeout (ms) for connect operation
#define CON_TIMEOUT      1000   // default timeout (ms) for connect operation
#define RW_TIMEOUT       10*1000   // default timeout (ms) for read/write operations
#define BUFFERSIZE       128*1024  // default ringbuffer size (bytes) for async read

#define NETDOWN_TIMEOUT  5        // timeout (s) for shutting down network
...
int RunCommand(const char *cmd, const char *State, const char *Name=0)
{
  ...
  if(cmd) {
    ...
    d(printf("music-sd: net: executing '%s'\n",tmp))
    res=SystemExec(tmp);
    free(tmp);
    }
  return res;
...
{
  d(printf("music-sd: net: netscript down delay\n"))
...
{
  d(printf("music-sd: net: name lookup %s\n",hostname))
  ...
  if(hp) {
    ...
    d(printf("music-sd: net: connecting to %s:%d\n",hostname,port))
    if(connect(fd,(struct sockaddr *)&sin,sizeof(sin))==0) {
      d(printf("music-sd: net: connected\n"))
      Done(1);
      }
    else { esyslog("music-sd: connect() failed: %s",strerror(errno)); Done(-1); }
    }
  else { esyslog("music-sd: Unknown host '%s'",hostname); Done(-1); }
}
...
:cRingBufferLinear(size>0?size:BUFFERSIZE,1,false)
{
  ...
  conTimeout=ConTimeoutMs ? ConTimeoutMs:CON_TIMEOUT * MP3Setup.ConnectTimeout;
#if APIVERSNUM >= 10132
  SetTimeouts(50,50);
#endif
}
...
bool cNet::Connect(const char *hostname, const int port)
{
  ...
  if(fd>=0) {
    ...
    if(res>0) {
      ...
      else esyslog("music-sd: fnctl() failed: %s",strerror(errno));
      }
    else if(res==0) esyslog("music-sd: Connection timed out");
    }
  else esyslog("music-sd: socket() failed: %s",strerror(errno));
  Disconnect();
  return false;
}
...
int cNet::Write(unsigned char *dest, int len)
{
  ...
  do {
    if(poll.Poll(rwTimeout)) {
      r=write(fd,dest,len);
      if(r<0 && errno!=EAGAIN) {
        esyslog("music-sd: write() failed: %s",strerror(errno));
        break;
        }
      dest+=r; len-=r; t+=r;
      }
    else { esyslog("music-sd: Write timed out"); break; }
    } while(len>0);
  return t;
}
...
int cNet::RingRead(unsigned char *dest, int len)
{
  ...
  for(;;) {
    if(!Available() && deferedErrno) {
      d(printf("music-sd: net: ringbuffer empty, async read bailed out\n"))
      return -1;
      }
    rd=Get(r);
    if(rd && r>0) {
...
{
  d(printf("music-sd: net: async read started\n"))
  ...
  while(connected) {
    if(poll.Poll(rwTimeout)) {
      ...
      if(r>0) {
        ...
        deferedErrno=errno;
        esyslog("music-sd: read() failed: %s",strerror(errno));
        break;
        }
      else if(r==0) {
        deferedErrno=-1;
        d(printf("music-sd: net: EOF from read()\n"))
        break;
        }
      }
    else {
      deferedErrno=-1;
      esyslog("music-sd: Read timed out");
      break;
      }
    }
  EnableGet();
