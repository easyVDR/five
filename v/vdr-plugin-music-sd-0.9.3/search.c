      ...
      if(basesource == newbase) {
	   ...
	   // Songname
        ...
        if( newsong && (newsong->Type()==otFile) ) {
          ...
          d(printf("music-sd: search: found '%s'\n", FullPath.c_str()));
...
eOSState cMP3SearchResult::AddOne(void) {
  ...
  if((item=CurrentItem())) {
    ...
    if(newpl->Load() && newpl->Count()) {
      isyslog("music-sd: added track %s to playlist %s", item->Name(),newpl->Name());
...
eOSState cMP3SearchResult::AddAll(void) {
  ...
  if(newpl->Load() && newpl->Count()) {
    isyslog("music-sd: added all tracks to playlist %s", newpl->Name());
    cMP3Control::SetPlayList(newpl);
...
:cOsdMenu(tr("Music-SD: Search"),30)
{
...
void cMP3Search::SaveSearch(void) {
  ...
  if(FILE* fd=fopen(Datei.c_str(), "w")) {
    ...
    fprintf(fd, query);
    d(printf("music-sd: search: Save query: \n%s\n",query));
    free(query);
...
printf("File exist: '%d'\n", Exist);
  ...
  if(InfoCache.SearchUp(Artist, Title , Genre, FullFile, YearFrom, YearTo, RateFrom, RateTo, Exist)) {
    ...
    d(printf("music-sd: search: %s of %i\n", buf, InfoCache.expr2));
...
void cMP3Search::UpdateMenu(int Which) {
  ...
  switch(which) {
    case 0: // ClearValues
      d(printf("music-sd: search: clear values\n"));
      ClearValues();    
      break;

    case 1: // PreviousSearch
      d(printf("music-sd: search: load previous search\n"));
      PreviousSearch();    
      break;

    case 2: // Get Genretest
      d(printf("music-sd: search: update value for genre\n"));
      updateGenre = false;
      SetGenre();           
      break;

    case 3: // Get RatingFrom
      d(printf("music-sd: search: update value for rating from\n"));
      updateRatingFrom = false;
      SetRatingFrom();
      break;

    case 4: // Get RatingTo
      d(printf("music-sd: search: update value for rating to\n"));
      updateRatingTo = false;
      SetRatingTo();
      break;

    case 5: // First Search
      d(printf("music-sd: search: first search\n"));
      NewValues();    
      break;
