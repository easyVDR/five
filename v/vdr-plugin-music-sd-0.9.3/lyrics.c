...
:cOsdMenu(tr("Music-SD: Local lyrics"))
{
  DisplayLocalLyrics();
  Display();
}
...
char *cMP3LoadLocalLyrics::CheckLocalLyrics(char *fileName) {
  ...
  if(fp) {
    fclose(fp);
    d(printf("music-sd[%d]: lyrics: lyrics %s found\n", getpid (), fileName));
    strcpy(tmpFile,fileName);
    result = tmpFile;
    }
  else
    d(printf("music-sd[%d]: lyrics: no lyrics found\n", getpid ()));
...
void cMP3LoadLocalLyrics::DisplayLocalLyrics() {
  ...
  d(printf("music-sd[%d]: lyrics: checking %s for lyrics\n", getpid (), Songname));
...
:cOsdMenu(tr("Music-SD: Extern lyrics"))
{
  DisplayExternLyrics();
...
char *cMP3LoadExternLyrics::CheckExternLyrics(const char *fileName) {
  ...
  if(fp) {
    fclose(fp);
    d(printf("music-sd[%d]: lyrics: file '%s' found\n", getpid (), fileName));
    strcpy(tmpFile,fileName);
    result = tmpFile;
    }
  else
    d(printf("music-sd[%d]: lyrics: file '%s' not found\n", getpid (), fileName));
...
int cMP3LoadExternLyrics::RunCommand(const char *cmd) {
  ...
  if(cmd) {
    char *tmp=0;
    asprintf(&tmp,"%s -a \"%s\" -t \"%s\"",cmd ,Artist.c_str(),Title.c_str()); 

    d(printf("music-sd[%d]: lyrics: executing '%s'\n",getpid (), tmp));
    res=SystemExec(tmp);
    free(tmp);
  }
  return res;
