...
#ifdef GUARD_DEBUG
  if(framebuff) {
    printf("music-sd: decoder-snd: bufferguard");
...
bool cSndDecoder::Start(void)
{
  ...
  if(file.Open() && info.DoScan(true)) {
    d(printf("music-sd: decoder-snd: open rate=%d frames=%lld channels=%d format=0x%x seek=%d\n",
    ...
    else esyslog("music-sd: ERROR: cannot play sound file %s: more than 2 channels",filename);
    }
  cDecoder::Unlock();
  return Clean();
}
...
bool cSndDecoder::Skip(int Seconds, float bsecs)
{
  ...
  if(playing && file.sfi.seekable) {
    ...
    d(printf("music-sd: decoder-snd: skip: secs=%d fsecs=%f frames=%lld current=%lld new=%lld\n",Seconds,fsecs,frames,file.Seek(0,true),newpos))
    ...
    if(frames>=0) {
      ...
      printf("music-sd: decoder-snd: skipping to %02d:%02d (frame %lld)\n",i/60,i%60,frames);
#endif
      res=true;
      }
    }
    ...
    while(fgets(line,sizeof(line),f)) {
      if(!state) {
        ...
        if(sscanf(line,CDFS_MARK_ID,&id,&n)==2) {
          d(printf("music-sd: decoder-snd: discid: found id=%08x n=%d\n",id,n))
        ...
        if(sscanf(line,CDFS_MARK_TR,&off,&end)==2) {
          dc(printf("music-sd: decoder-snd: discid: found offset=%d end=%d for track %d\n",off,end,tr+1))
          offsets[tr]=off+CDFS_TRACK_OFF;
          if(++tr==ntrks) {
            nsecs=(end+1)/75;
            dc(printf("music-sd: decoder-snd: discid: nsecs=%d / 0x%x\n",nsecs,nsecs))
            res=true;
            break;
            }
          }
...
bool cCDDBDisc::Load(cDiscID *id, const char *filename)
{
  ...
  d(printf("music-sd: decoder-snd: cddb: loading discid %08x from %s\n",id->discid,filename))
  DiscID=id->discid;
  ...
  if(f) {
    ...
    if(Count()>0) {
      ...
      d(printf("music-sd: decoder-snd: cddb: found artist='%s' album='%s' genre='%s' isSampler=%d\n",Artist,Album,Genre,isSampler))
      free(DTitle); DTitle=0;

      if(!isSampler && Artist && Album && !strncmp(Album,Artist,strlen(Artist))) {
        d(printf("music-sd: decoder-snd: cddb: detecting sampler from Artist==Album\n"))
        isSampler=true;
        }
      ...
      if(!isSampler) {
        ...
        if(nofail1==Count() || nofail2==Count()) {
          d(printf("music-sd: decoder-snd: cddb: detecting sampler from nofail\n"))
          isSampler=true;
          }
        }
      ...
      free(ExtD); ExtD=0;
      d(printf("music-sd: decoder-snd: cddb: found year=%d\n",Year))
      ...
      while(s) {
        ...
        d(printf("music-sd: decoder-snd: cddb: found track %d title='%s' artist='%s'\n",s->Track,s->Title,s->Artist))
        s=Next(s);
        }
      return true;
      }
...
bool cCDDB::RemoteGet(cDiscID *id)
{
  ...
  if(net->Connect(MP3Setup.CddbHost,MP3Setup.CddbPort)) {
    ...
    if(code/100==2) {
      ...
      code=DoCddbCmd("cddb hello %s %s %s %s\n",user,host,"vdr-plugin-music-sd", "0.3.0-rc2");
      if(code/100==2) {
            ...
            if(cat) {
              ...
              if(code==210) {
                ...
                if(MakeDirs(name,false)) {
                  ...
                  else esyslog("music-sd: fopen() failed: %s",strerror(errno));
                  }
                free(name);
                }
              else if(code>0) esyslog("music-sd: server read error: %d %s",code,cddbstr);
              free(cat);
              }
            }
          else if(code>0) esyslog("music-sd: server query error: %d %s",code,cddbstr);
          }
        else esyslog("music-sd: server proto error: %d %s",code,cddbstr);
        }
      else if(code>0) esyslog("music-sd: server hello error: %d %s",code,cddbstr);
      }
    else if(code>0) esyslog("music-sd: server sign-on error: %d %s",code,cddbstr);
    }
...
#ifdef CDDB_DEBUG
    if(log) printf("music-sd: decoder-snd: cddb: <- %s",buff);
#endif
    return true;
    }
  return false;
...
int cCDDB::GetCddbResponse(void)
{
  ...
  if(GetLine(buf,sizeof(buf))) {
    ...
    else esyslog("music-sd: Unexpected server response: %s",buf);
    }
  return -1;
}
...
int cCDDB::DoCddbCmd(const char *format, ...)
{
  ...
  va_end(ap);
#ifdef CDDB_DEBUG
  printf("music-sd: decoder-snd: cddb: -> %s",buff);
#endif
  int r=net->Puts(buff);
  free(buff);
  if(r<0) return -1;
...
bool cSndInfo::CDDBLookup(const char *filename)
{
  if(id->Get()) {
    ...
    if(s && sscanf(s+strlen(CDFS_TRACK),"%d",&tr)==1) {
      d(printf("music-sd: decoder-snd: looking up disc id %08x track %d\n",id->discid,tr))
...
void cSndFile::Error(const char *action)
{
  ...
  esyslog("music-sd: ERROR: sndfile %s failed on %s: %s",action,Filename,buff);
}
