...
:cOsdMenu(tr("Music-SD: Timer Shutdown"),40)
{
...
:cOsdMenu(tr("Music-SD: Change time to skip"),20)
{
...
:cOsdMenu(tr("Music-SD: Add new target directory"),20)
{
  ...
  if(AddDir) {
    ...
    if(filestr) {
      ...
      if(mkdir(AddDir, FileInfo.st_mode) == -1) {
        if(errno == EEXIST) esyslog("music-sd: options: '%s' already exist", AddDir);
      }    
    }
    else
      esyslog("music-sd: options: could not open '%s'", datei.c_str());	  
  }
}
...
:cOsdMenu(tr("Music-SD: Goal listing"))
{
...
void cMP3CopyDir::GetCopyDirs()
{
  ...
  if(filestr) {
    ...
    esyslog("music-sd: options: ERROR: no directories defined! check '%s'", datei.c_str());
...
:cOsdMenu(tr("Music-SD: Quick settings"))
{
  SetHelp();
  ...
  if (current==0) { 
    if(MP3Setup.EnableShutDown) {
      if(Interface->Confirm(tr("Disable automatic shutdown ?"))) {
        ...
        return AddSubMenu(new cMenuText(tr("Music-SD: Automatic shutdown"), tr("Automatic shutdown disabled !"), fontFix));
      }
    }
    else {
      if(Interface->Confirm(tr("Enable automatic shutdown ?"))) {
        MP3Setup.EnableShutDown = 1;  
        return AddSubMenu(new cMenuText(tr("Music-SD: Automatic shutdown"), tr("VDR will shutdown after player stopped !"), fontFix));
      }
    }
  }
  ...
  if (current == 3) {
    ...
    if(!mgr->ShuffleMode()) {
      ...
      if(Interface->Confirm(tr("Disable shuffle mode only for this session ?")) ) {
        ...
        Plugin->SetupStore("InitShuffleMode", 1);
 #ifdef DEBUG
        printf("music-sd: options: ShuffleMode = %d\n", mgr->ShuffleMode());
        printf("music-sd: options: InitShuffleMode = %d\n", MP3Setup.InitShuffleMode);
#endif
        return osBack;
        }
      else {  
  ...
  if (current == 4) {
    ...
    if(!mgr->LoopMode()) {  
      ...
      if(Interface->Confirm(tr("Disable loop mode only for this session ?"))) {
        ...
        Plugin->SetupStore("InitLoopMode", 1);
#ifdef DEBUG
        printf("music-sd: options: LoopMode = %d\n", mgr->LoopMode());
        printf("music-sd: options: InitLoopMode = %d\n", MP3Setup.InitLoopMode);
#endif
        return osBack;
      }
      else {
