...
void cOggFile::Error(const char *action, const int err)
{
  ...
  esyslog("music-sd: ERROR: vorbisfile %s failed on %s: %s",action,Filename,errstr);
}
...
bool cOggInfo::DoScan(bool KeepOpen)
{
  ...
  if(vc) {
    for(int i=0 ; i<vc->comments ; i++) {
      ...
      d(printf("music-sd: decoder-ogg: comment%d='%s'\n",i,cc))
      ...
      if(p) {
        ...
        else if(!strncasecmp(cc,"GENRE",len)) {
          if(!Genre) Genre=strdup(p);
          }
        else if(!strncasecmp(cc,"COMMENT",len)) {
          if(!Comment) Comment=strdup(p);
          }
        ...
        else if(!strncasecmp(cc,"DATE",len)) {
          if(Year<0) {
            Year=atoi(p);
            if(Year<1800 || Year>2100) Year=-1;
            }
  ...
  if(!vi) Abort(false);
  d(printf("music-sd: decoder-ogg: info ch=%d srate=%ld brate_low=%ld brate_high=%ld brate_avg=%ld\n",
...
bool cOggDecoder::Start(void)
{
  ...
  if(file.Open() && info.DoScan(true)) {
    d(printf("music-sd: decoder-ogg: open rate=%d channels=%d seek=%d\n",
             info.SampleFreq,info.Channels,file.CanSeek()))
    if(info.Channels<=2) {
      Unlock();
      return true;
      }
    else esyslog("music-sd: ERROR: cannot play ogg file %s: more than 2 channels",filename);
    }
  Clean();
  Unlock();
  return false;
...
bool cOggDecoder::Skip(int Seconds, float bsecs)
{
  ...
  if(playing && file.CanSeek()) {
    ...
    d(printf("music-sd: decoder-ogg: skip: secs=%d fsecs=%f current=%lld new=%lld\n",Seconds,fsecs,file.IndexMs(),newpos))
...
#ifdef DEBUG
      int i=index/1000;
      printf("music-sd: decoder-ogg: skipping to %02d:%02d\n",i/60,i%60);
#endif
      res=true;
      }
    }
