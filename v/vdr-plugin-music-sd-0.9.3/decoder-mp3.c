...
:cDecoder(Filename)
{
  ...
  if(preinit) {
    //d(printf("music-sd: decoder-mp3: preinit\n"))
...
bool cMP3Decoder::Start(void)
{
  ...
  if(str->Open() && scan->DoScan(true)) {
    if(!isStream) {
      ...
      if(!fi) esyslog("music-sd: ERROR: no memory for frame index, rewinding disabled");
      }
    Unlock();
    return true;
    }
...
eDecodeStatus cMP3Decoder::DecodeError(bool hdr)
{
  if(stream->error==MAD_ERROR_BUFLEN || stream->error==MAD_ERROR_BUFPTR) {
    ...
    d(printf("music-sd: decoder-mp3: mad: decode %sfailed, frame=%d: %s\n",hdr?"hdr ":"",framenum,mad_stream_errorstr(stream)))
    return dsError;
    }
  else { 
    ...
    errcount+=hdr?1:100;
    d(printf("music-sd: decoder-mp3: mad: decode %serror, frame=%d count=%d: %s\n",hdr?"hdr ":"",framenum,errcount,mad_stream_errorstr(stream)))
    }
  return dsOK;
}
...
struct Decode *cMP3Decoder::Decode(void)
{
  ...
  while(playing) {
    if(errcount>=MAX_FRAME_ERR*100) {
      esyslog("music-sd: ERROR: excessive decoding errors, aborting file %s",filename);
      return Done(dsError);
      }
    ...
    if(mad_header_decode(&frame->header,stream)<0) {
      ...
      if(!isStream) {
#ifdef DEBUG
        if(framenum>=framemax) printf("music-sd: decoder-mp3: framenum >= framemax!!!!\n");
#endif
...
void cMP3Decoder::MakeSkipTime(mad_timer_t *skiptime, mad_timer_t playtime, int secs, float bsecs)
{
  ...
  *skiptime=playtime;
  ...
  mad_timer_add(skiptime,time);
  d(printf("music-sd: decoder-mp3: skip: playtime=%ld secs=%d full=%d bsecs=%f skiptime=%ld\n",
  ...
  if(playing && !isStream) {
    if(!mad_timer_compare(skiptime,mad_timer_zero)) { // allow only one skip at any time
      ...
      if(mad_timer_compare(playtime,time)<=0) { // forward skip
        ...
        printf("music-sd: decoder-mp3: forward skipping to %02d:%02d\n",i/60,i%60);
#endif
        ...
        if(fi) {
#ifdef DEBUG
          int i=mad_timer_count(time,MAD_UNITS_SECONDS);
          printf("music-sd: decoder-mp3: rewinding to %02d:%02d\n",i/60,i%60);
#endif
          ...
          printf("music-sd: decoder-mp3: new playtime=%d framenum=%d filepos=%lld\n",i,framenum,fi[framenum].Pos);
#endif
          res=true;
          }
        }
...
bool cScanID3::DoScan(bool KeepOpen)
{
  ...
  if(str->Filesize>=128 && str->Seek(str->Filesize-128)) {
    ...
    if(str->Stream(data,len)) {
      ...
      if(tag) {
        d(printf("music-sd: decoder-mp3: id3-scan: initialy found ID3 V1 tag at EOF\n"))
        ParseID3(tag);
  ...
  while(1) {
    if(*urgent) {
      d(printf("music-sd: decoder-mp3: id3-scan: urgent request, aborting!\n"))
    ...
    if(mad_header_decode(&header,&stream)<0) {
      if(stream.error==MAD_ERROR_BUFLEN || stream.error==MAD_ERROR_BUFPTR) {
        ...
        memcpy(buf,stream.this_frame,8); buf[8]=0;
        memcpy(buf2,stream.this_frame,8);
        printf("music-sd: decoder-mp3: id3-scan: lost sync %08x %08x %s\n",buf2[0],buf2[1],buf);
#endif
	  ...
	  if(tag) {
        ...
        d(printf("music-sd: decoder-mp3: id3-scan: decode header error (frame %d): %s\n",Frames,mad_stream_errorstr(&stream)))
        errcount++;
  ...
  if(res) {
    d(printf("music-sd: decoder-mp3: mad: scanned %d frames%s\n",Frames,Frames?"":"(is this really a mp3?)"))
    if(Frames) {
      ...
      if(!is_vbr) {
        d(printf("music-sd: decoder-mp3: mad: constant birate\n"))
        ...
        d(printf("music-sd: decoder-mp3: mad: vbr, but has Xing frame\n"))
        ...
        d(printf("music-sd: decoder-mp3: mad: vbr detected\n"))
...
struct id3_tag *cScanID3::GetID3(struct mad_stream *stream, id3_length_t tagsize) const
{
  ...
  if(count>=tagsize) {
    ...
    if(!(allocated=(id3_byte_t *)malloc(tagsize))) {
      esyslog("music-sd: ERROR: not enough memory for id3 tag buffer");
      return 0;
      }
    ...
    while(count<tagsize) {
      ...
      if(!str->Stream(sdata,slen) || !slen) {
         d(printf("music-sd: decoder-mp3: mad: error or eof on ID3 tag parse\n"))
         free(allocated);
         return 0;
         }
...
{
  d(printf("music-sd: decoder-mp3: id3-scan: parsing ID3 tag\n"))
  ...
  ParseCom(tag,ID3_FRAME_COMMENT,Comment);
...
void cScanID3::ParseCom(const struct id3_tag *tag, const char *id, char * &data)
{
  unsigned int i;
  const id3_ucs4_t *ucs4;
...
void cScanID3::ParseRating(const struct id3_tag *tag, const char *id)
{
  unsigned int i;
  ...
  while ((frame=id3_tag_findframe(tag,"POP",i++))) {
    rating=id3_field_getint(id3_frame_field(frame,1));
    break;
  }    
}
