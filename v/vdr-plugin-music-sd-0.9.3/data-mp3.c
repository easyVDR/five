    ...
    while(fgets(buffer,sizeof(buffer),f)>0) {
      if(buffer[0]=='#') {
        if(!strncmp(buffer,WINAMPEXT,strlen(WINAMPEXT))) {
          d(printf("music-sd: data-mp3: detected WinAmp style playlist\n"))
          isWinAmp=true;
          }
        continue;
        }
      if(!isempty(buffer)) {
        ...
        if(song->Parse(buffer,obj->Subdir())) Add(song);
          ...
          esyslog("music-sd: error loading playlist %s\n",obj->FullPath());
          delete song;
          result=false;
          break;
          }
...
bool cInstantPlayList::Load(void)
{
  ...
  switch(obj->Type()) {
    case otFile:
      d(printf("music-sd: data-mp3: instant: file %s\n",obj->Name()))
      if(strcasecmp(obj->Name(),basename)) {
        d(printf("music-sd: data-mp3: instant: detected as playlist\n"))
        res=cPlayList::Load();
        }
      else {
        Add(new cSong(obj));
    ...
    case otDir:
      {
      d(printf("music-sd: data-mp3: instant: dir %s\n",obj->Name()))
      res=ScanDir(obj->Source(),obj->Path(),stFile,obj->Source()->Include(),excl_pl,true);
      Sort();
      break;
      }
    case otBase:
      d(printf("music-sd: data-mp3: instant: base\n"))
