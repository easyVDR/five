...
:cOsdMenu(tr("Music-SD: Rating"),15)
{
  Song = current;
  GetRatingText();
  SetHelp();
...
eOSState cMP3Rating::SetRating(void) {
  ...
  if(MP3Setup.RateInFile) {
    if( FILE* fd=fopen(Song->Fullname(), "r")) {
      ...
      d(printf("music-sd: rating: Edit ID3Tag 'POPM' to '%s:%i'\n",MP3Setup.RateCompat,rating));
      asprintf(&cmd, "%s --POPM '%s:%i:0' '%s'", Datei.c_str(), MP3Setup.RateCompat, rating, Song->Fullname());
      isyslog("music-sd: rating: Executed '%s'\n", cmd);
      system(cmd);
      free(cmd);
      
      mgr->UpdateSong(Artist, Album, Title, Genre, Year, rating, Song);
      d(printf("music-sd: rating: Update Rating to '%i'\n", rating));      
    }
  }
  else {
