...
bool cFileIO::Open(void)
{
  ...
  if(FileInfo(log)) {
    ...
    else if(log) { esyslog("music-sd: ERROR: open failed on %s: %s",Filename,strerror(errno)); }
    }
  Close();
  return false;
}
...
int cFileIO::Read(unsigned char *Data, int Size)
{
  ...
  if(r>=0) readpos+=r;
  else if(log) { esyslog("music-sd: ERROR: read failed in %s: %s",Filename,strerror(errno)); }
  return r;
}
...
bool cFileIO::Seek(unsigned long long pos, int whence)
{
  if(pos>=0 && pos<=Filesize) {
    ...
    if((readpos=lseek64(fd,pos,whence))>=0) {
      if(readpos!=pos) { dsyslog("music-sd: seek mismatch in %s, wanted %lld, got %lld",Filename,pos,readpos); }
      return true;
      }
    else if(log) { esyslog("music-sd: ERROR: seek failed in %s: %s",Filename,strerror(errno)); }
    }
  else d(printf("music-sd: netstream: bad seek call fd=%d pos=%lld name=%s\n",fd,pos,Filename))
  return false;
}
#endif
  ...
  if(FileInfo(log)) {
    if((fd=open(Filename,O_RDONLY))>=0) {
      ...
      if(Filesize<=MAX_MMAP_SIZE) {
        ...
        else dsyslog("music-sd: mmap() failed for %s: %s",Filename,strerror(errno));
        }
#endif

        buffer = new unsigned char[MP3FILE_BUFSIZE];
        if(buffer) return true;
        else { esyslog("music-sd: ERROR: not enough memory for buffer: %s",Filename); }
      }
    else if(log) { esyslog("music-sd: ERROR: failed to open file %s: %s",Filename,strerror(errno)); }
    }
...
bool cStream::Seek(unsigned long long pos)
{
  if(fd>=0 && pos>=0 && pos<=Filesize) {
    ...
    if(ismmap) {
      ...
      else { esyslog("music-sd: ERROR: seeking failed in %s: %d,%s",Filename,errno,strerror(errno)); }
      }
    }
  else d(printf("music-sd: netstream: bad seek call fd=%d pos=%lld name=%s\n",fd,pos,Filename))
  return false;
  ...
  if(fd>=0) {
    if(readpos<Filesize) {
      if(ismmap) {
        ...
        else { esyslog("music-sd: ERROR: read failed in %s: %d,%s",Filename,errno,strerror(errno)); }
        }
      }
    else {
      len=0;
...
bool cNetStream::ParseURL(const char *line, bool log)
{
  ...
  if(r==2) {
    d(printf("music-sd: netstream: adding default path '/'\n"))
    strcpy(p,"/");
    r++;
    }
  if(r==3) {
    ...
    d(printf("music-sd: netstream: parsed proto='%s' host='%s' path='%s' auth='%s'\n",pr,h,p,a))
    ...
    else if(log) esyslog("music-sd: Unsupported protocol %s in: %s",pr,line);
    }
  else if(log) esyslog("music-sd: Bad URL line: %s",line);
  return false;    
}
...
bool cNetStream::ParseURLFile(const char *name, bool log)
{
  ...
  if(f) {
    ...
    else if(log) esyslog("music-sd: Nothing to read from URL file %s. File empty?",name);
    fclose(f);
    }
  else if(log) esyslog("music-sd: fopen() failed on URL file %s: %s",name,strerror(errno));
  return res;
}
...
bool cNetStream::SendRequest(void)
{
  ...
  if(net->Connect(MP3Setup.UseProxy ? MP3Setup.ProxyHost:host , MP3Setup.UseProxy ? MP3Setup.ProxyPort:port)) {
    d(printf("music-sd: netstream: -> %s",buff))
...
bool cNetStream::ParseHeader(const char *buff, const char *name, char **value)
{
  ...
  if(s && !strncasecmp(buff,name,s-buff)) {
    s=skipspace(s+1);
    d(printf("music-sd: netstream: found header '%s' contents '%s'\n",name,s))
...
bool cNetStream::GetHTTPResponse(void)
{
  ...
  while(net->Gets(buff,sizeof(buff))>0) {
    ...
    Dump(DUMP_HEAD,buff);
#endif
    d(printf("music-sd: netstream: <- %s\n",buff))
    hcount++;
    if(hcount==1) {   // parse status line
      if(sscanf(buff,"%*[^ ] %d %128s",&code,text)!=2) {
        esyslog("music-sd: Bad HTTP response '%s' from %s:%d",buff,host,port);
        goto out;
        }
      }
      ...
      if(buff[0]==0) { // headers finish if we receive a empty line
        switch(code) {
          ...
          case 302: // MOVED_TEMPORARILY
             ...
             else esyslog("music-sd: No location header for redirection from %s:%d",host,port);
             goto out;
          default:
             esyslog("music-sd: Unhandled HTTP response '%d %s' from %s:%d",code,text,host,port);
             goto out;
          }
        }
      ...
      if(ParseHeader(buff,"icy-metaint",&meta)) {
        ...
        d(printf("music-sd: netstream: meta interval set to %d\n",metaInt));
        }
      free(meta);
      }
    }
...
bool cNetStream::Open(bool log)
{
  ...
  if(ParseURLFile(Filename,log)) {
    ...
    else esyslog("music-sd: Not enough memory for buffer");
    }
  ...
  if(s && !strncasecmp(buff,name,s-buff)) {
    ...
    if(s[1]=='\'' && end) {
      *end=0;
      ...
      if(strlen(s)>0) {
        d(printf("music-sd: netstream: found metadata '%s' contents '%s'\n",name,s))
        free(*value); *value=strdup(s);
        }
      return end+1;
      }
    else d(printf("music-sd: netstream: bad metadata format\n"))
    }
