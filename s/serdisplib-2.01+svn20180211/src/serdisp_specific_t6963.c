...
void serdisp_t6963_checkready(serdisp_t* dd) {
  ...
  uint32_t td_clk1 =          SIG_CD         ;
  uint32_t td_clk2 = SIG_CE | SIG_CD | SIG_RD;
  uint32_t td_clk3 =          SIG_CD         ;

  int cnt = 0;

  /* check, if at least one of D0, D1, or D3 is high */
  uint32_t rc;     /* returned status byte */
  uint32_t rc_check = SIG_D0 | SIG_D1 | SIG_D3;
  ...
  if (cnt > 1)   
    sd_debug(1, "serdisp_t6963_checkready(): delay too small? (cnt = %d; rc = 0x%08x)", cnt, rc);
}
...
void serdisp_t6963_transfer(serdisp_t* dd, int iscmd, byte item) {
  if (serdisp_t6963_internal_getStruct(dd)->interfacemode == INTERFACE_PARALLEL) {
    ...
    /* active-low signals are internally seen active-high because they will be auto-inverted later if needed */
    uint32_t td_clk1 = 0;
    uint32_t td_clk2 = SIG_WR | SIG_CE;
    uint32_t td_clk3 = 0;
    uint32_t td_clk4 = 0;
    int i;
