...
/*
 * Hardware function: Enters graphical mode
 */
 /*
static int yard_hwEnterGmode(Driver *drvthis)
{
	...
	cmdBuf[0] = YICMD_LCDWRITERAWEX;
	cmdBuf[1] = 0x02;
	cmdBuf[2] = 0xF2;
	cmdBuf[3] = 'c';
	return yard_hwWrite(drvthis, cmdBuf, 4);	
}
*/
/*
...
/*
		case Dmode_graphical: {
			if ( (x > p->gwidth) || (x < 1) || (y > p->gheight) || (y < 1) )
				return -1;
			
			x = (x - 1) * (p->bigcellwidth + p->hspace);
			y = p->bigcellheight + (y - 1) * (p->bigcellheight + p->vspace);
         ...
         // Setup and send YARD command
			...
			cmdBuf[3] = x;
			cmdBuf[4] = y;
			return yard_hwWrite(drvthis, cmdBuf, 5);
...
yard_init(Driver *drvthis)
{
	...
	p->LCDtype = 0; //0=HD44780; 1=KS0066;2=T6963c;3=KS0108
...
/*
		case Dmode_graphical: {
			// Set width and height for graphical mode
			width = p->gwidth;
			height = p->gheight;
			break;
		}
...
yard_string(Driver *drvthis, int x, int y, const char string[])
{
	...
	// Change display mode if necessary; Only char mode is supported right now
