 ...
 *      20.09.15 Old USB-ID by John Bigboteh: http://www.easy-vdr.de/forum/index.php?topic=13723.msg168963#msg168963
 */
#include <inttypes.h>
#include <stdio.h>
#include <stdlib.h>
...
int usb_dev_id = USBDEV_SHARED_PRODUCT;
...
int main(int argc, char **argv) {
	...
	while ((opt = getopt(argc, argv, "hVdp:r:si")) != -1) {
		...
		case 's':
			...
			break;
		case 'u':
			usb_dev_id = USBDEV_SHARED_PROD_OLD;
			break;
		case 'i':
			ignore_dev = 1;
			break;
...
void usage(int exval) {
	...
	printf("  -u              use a old DEVICE id 0x05df\n");
	...
	for (bus = usb_busses; bus; bus = bus->next) {
		for (dev = bus->devices; dev; dev = dev->next) {
			if (dev->descriptor.idVendor == USBDEV_SHARED_VENDOR
					&& dev->descriptor.idProduct == usb_dev_id) {
				char string[256];
				int len;
